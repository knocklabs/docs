...
### Batching for a fixed duration

You can set a fixed duration that the batch will be open using the "Batch for a fixed window" option in the batch step. The window accepts a relative duration, which can be specified as seconds, minutes, hours, or days.

The batch will be scheduled to close when the batch step is first triggered for the recipient, plus the duration specified.

### Batching for a dynamic interval

Your batches can also be driven by a dynamic interval, which can be provided on the data, recipient, actor, or environment variables associated with the workflow run.
...
**Please note**: the open batch window will never be extended by a subsequent workflow trigger with a different dynamic batch window specified. The first workflow trigger without an open batch will always determine the batch window close.
