...
Conditions can also be specified at channel environment settings level. For this case, when evaluating a channel step, the conditions for the channel on the environment for which the
workflow trigger was generated will be evaluated.
...
- Only send an email if environment is development and email matches a particular domain

## Building trigger conditions

### Building conditions at the step level
...
### Building conditions at the channel level
Trigger conditions are created and edited on each channel environment setting and can be accessed by visiting the "Conditions" tab on the channel environment settings modal.
...
- **Not sent**: triggered if the message status is not `sent`
- **Sent**: triggered if the message status is `sent` or `delivered`
...
<Callout
  ...
  text={
    <>
      <span className="font-bold">Available status vary per channel type.</span> Message status trigger conditions can be configured
      to use any type of channel steps, but not all messages status are available on all of them. The in-app feed channels support
      all statuses, the rest of the channels only support the "Not Sent" and "Sent" statuses. Message status trigger conditions
      are <span className="font-bold">only</span> available for step level conditions.
    </>
  }
/>
...
## Example: conditionally sending an email on staging environments

Another use-case for trigger conditions is conditionally sending emails on staging environments to email addresses in your domain.

In order to implement this, you will need to add a condition to the channel environment settings of your email channel.

<figure>
  <img
    src="/images/functions/channel-level-conditions.png"
    alt="Setting a trigger condition on the email channel environment settings"
  />
  <figcaption>
    Setting a trigger condition on the email channel environment settings that
    checks your company's subdomain
  </figcaption>
</figure>

With this condition in place, emails will only go out for recipients whose email contains knock.app whenever that email
channel is used on any of our workflows.
