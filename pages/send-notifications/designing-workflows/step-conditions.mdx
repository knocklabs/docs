...
tags: ["triggers", "conditions", "conditionals", "steps", "routing", "conditional send"]
...
<Callout
  ...
  text={
    <>
      <span className="font-bold">Channel-level conditions.</span> You can also configure conditions for an entire channel at the environment-level. When a channel-level conditions is configured in a given environment, all instances of that channel in your workflows will include the condition. 
      <br/>
      <br/>
...
<Callout
  ...
  text={
    <>
      <span className="font-bold">Available statuses vary per channel type.</span> It's important to note that while you can reference the status of any message step in a message status condition, not all channel types contain the same available message statuses. The in-app feed channels support
      all statuses, the rest of the channels only support the "Not Sent" and "Sent" statuses.
    </>
  }
/>
...
<figure>
  ...
  />
  <figcaption>
    Setting a condition on the email step for when the in-app
...
Another use-case for conditions is conditionally sending emails on staging environments to email addresses in your domain.
...
<figure>
  <img
    ...
    alt="Setting a condition on the email channel environment settings"
  />
  <figcaption>
    Setting a condition on the email channel environment settings that
