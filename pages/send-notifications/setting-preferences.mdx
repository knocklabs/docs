...
Preferences live under a [user](/send-and-manage-data/users) or an [object](/send-and-manage-data/objects) and are used to structure a set of rules about what notifications a recipient should receive (called a `PreferenceSet`). They are designed to be flexible and expressive to accommodate complex scenarios like per-resource, and channel preferences (like opting out of an individual Slack channel).
...
In the case in which the per-tenant preference set doesn't exist, a [default is selected](/send-and-manage-data/preferences#preference-defaults).
...
**Please note**: setting preferences inline will always upsert the preference set, meaning that changes can and will be overriden. Generally, we recommend against using inline preferences, where possible, to ensure the integrity of your data.
