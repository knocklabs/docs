...
<Callout
  ...
  text={
    <>
      ...
      Knock debugger to see each of these workflows and their step-by-step
      execution.
      <br />
      <br />
      In the Knock dashboard, go to the Developers page, click Debugger, and then
...
By default, a batch function batches all incoming notifications **per recipient**. If you do not provide a batch key, this is the behavior your batch function will default to.

You may want to batch a recipient's notifications into subsets based on the contents of a given notification. In these cases you can provide a batch key to tell Knock to batch each recipient's notifications by a batch key. 

<Callout
  emoji="🌠"
  text={
    <>
      <span className="font-bold">Batching mental model tip.</span> Here's a helpful way to think about batch. By default the batch function batches on a key of <code>recipient_id</code>. When a batch key is provided, it batches on a key of <code>concat(recipient_id, batch_key)</code>.  
  }
/>
