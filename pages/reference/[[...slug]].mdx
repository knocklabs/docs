...
all preferences are opted in unless a preference explictly opts the user out of the notification.

Preferences are always executed as channel types and then workflows. That means that if a channel type
is set to opt the user out then no notifications will be sent on that channel.
...
Retrieve a users preference object. Will always return an empty preference object,
...
Sets preferences within the given preference set. This is a destructive operation and this
