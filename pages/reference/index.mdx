...
Identifies up to 100 users at a time. Returns a `BulkOperation` that executes the job asyncronously. Progress can be tracked via the BulkOperation API.
...
Deletes up to 100 users at a time. Returns a `BulkOperation` that executes the job asyncronously. Progress can be tracked via the BulkOperation API.
...
Sets the preferences for the users indicated via an asyncronous `BulkOperation`. This is a destructive operation and
will replace any existing preferences for the users given with the preferences sent.
...
<Attributes>
  ...
  <Attribute
    name="preferences.id"
    type="string"
    description="The preference set id"
  />
  <Attribute
...
The bulk change endpoint allows you to modify all, or a subset of messages for a given channel via an asyncronous job. The messages
...
field due to the asyncronous nature of the operation. If you wish to pin this job to a specific point in
