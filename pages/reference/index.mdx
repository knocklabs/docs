...
- `actor_missing` - You called a workflow with an actor that has not yet been identified in Knock. To resolve, identify the actor prior to passing their ID in a workflow trigger. 
- `api_key_expired` - Your API key has expired. You can regenerate one on the developers page of the Knock dashboard. 
- `api_key_missing` - The API key you provided is invalid. 
- `workflow_inactive` - The workflow you attempted to invoke is marked as inactive. To resolve this error, activate the workflow on its page in the dashboard. 
- `workflow_missing` - The workflow you attemped to invoke could not be found. To resolve this error, confirm that you're calling the correct environment and that your workflow has been committed to it. 
