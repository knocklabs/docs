...
You can optionally include detailed metadata about users or objects in the `recipients` and `actor` fields in a notify call. When present, Knock will persist the data for the respective users or objects as part of processing the workflow. This is comparable to direct calls to the [identify user](#identify-user) or [set object](#set-object) APIs.
...
<Attributes>
  <Attribute
    name="recipients"
    type="Recipient[]"
    description="A list of maps of user or object metadata to identify and notify for this workflow"
  />
  <Attribute
    name="actor"
    type="Recipient (optional)"
    description="A map of user or object metadata to identify as who or what performed this action"
  />
  <Attribute
    name="cancellation_key"
    type="string (optional)"
...
<MultiLangCodeBlock snippet="workflows.trigger-with-identification" title="Trigger a workflow with inline identifications" />
