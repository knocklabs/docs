...
<Callout
  ...
  text={
    <>
      <span className="font-bold">This feature is in limited release.</span>{" "}
      Idempotence is currently only available for the{" "}
      <code>POST /v1/workflows/:key/trigger</code> endpoint.
    </>
  }
/>
...
<Attributes>
  ...
  <Attribute
    ...
    type="datetime"
    description="A UTC datetime string that indicates when the schedule will next run, generated from the repeat rules."
  />
</Attributes>
...
<Section title="Recipient schedules" slug="get-recipient-schedules">
<ContentColumn>

Returns a paginated list of schedules for a single recipient, in descending order.
