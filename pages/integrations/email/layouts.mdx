...
The **layout** includes the header and footer of your email, as well as any other HTML or CSS that will be used across all (or multiple) templates. You can think of your email layout as the wrapper of your email notifications.
...
Here's an example of a transactional email we send at Knock. The area shaded in green is the template. The area shaded in blue is the layout.
...
Click on the "Default" layout in your layouts list to enter the layout editor. You'll start by looking at the pre-built layout that Knock gives you out of the box. You'll find a footer design option in this visual builder where you can add footer links to this pre-built layout. You can update the logo, icon, and brand color of your email layout by going to "Settings > Branding" and updating these attributes.

If you'd rather create your own layout from scratch, you can click "edit in code editor" in the top right corner of the layout editor to enter our custom layout editor. You can learn more about custom layouts in the [custom layouts and styling section](/layouts#custom-layouts-and-styling) of this guide.

## Working with multiple layouts
You can create multiple layouts in Knock for cases when you want different email notifications to have different styling. As an example, you may want to use a plaintext email from your founder when new users sign up for your application.
...
<Callout
  ...
  text={
    <>
      <span className="font-bold">Note:</span> Email layouts follow the Knock environment commit model, so you'll need to commit them to your current environment before you'll see them appear in your email notifications.
    </>
  }
/>

## Custom layouts and styling
If you want to use your own custom email layout editor, you can go into the layout editor and click "Edit in code editor" to go to an HTML and CSS editor for your email layout. The important thing to remember here is that your layout needs a `{{content}}` somewhere in its `body` tag for the email template to inject into the layout.

### Creating a plaintext email layout
If you want to create a simple plaintext email layout with no formatting whatsoever, you'd just create an email layout that contains the text `{{content}}`. That's it.
...
### Custom layouts and the Knock visual template editor

When you select a custom layout to use on a given email template, you'll still be able to use the visual template editor for inserting markdown, HTML, and button elements into the `content` block within your layout.

[Need Thomas help on this one.]
