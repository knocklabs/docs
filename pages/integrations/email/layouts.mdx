...
### Defining pre-content

Before your emails are sent to your recipients we compile your templates and your layouts together to create a **single email template**.
...
Per email template you can optionally set `pre-content` that will be injected at the very top of your email template during email compilation. You can set and edit the `pre-content` content in the three dot menu at the top right of the email template editor.

#### Using `pre-content` content

You can use `pre-content` to:
...
<Callout
  ...
  text={
    <>
      <span className="font-bold">Note:</span> by default your pre-content content will be injected into both the text and HTML versions of your template. As such you should be careful to only use this space to declare variables and not output content.
    </>
  }
/>

#### Example of `pre-content` usage

You need to inject a custom previewText at the top of your email layout before my template in your email layout. You need this to be the first thing that’s rendered so that email clients like Gmail correctly show the preview text associated with your message.
...
You can define `previewText` in the `pre-content` content like this:

```
{% assign previewText = "Hello and welcome to Knock" %}
```

Which will result in your template te be rendered like this:
