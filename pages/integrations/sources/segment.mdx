...
You can add an event as a trigger to your workflow directly from the workflow builder. Click on the workflow's trigger step and change the type from "API" to "Event". Then you'll be able to select the event and map its properties into the data the workflow needs.
...
<Callout
  ...
  text={
    <>
      ...
      if you send Knock an event that includes a recipient that has not yet been identified in Knock, our system will not generate a workflow run for that user. <br/><br/>
      For use cases such as new signup events, where events will often reach Knock before identify calls, consider <a href="#inline-identify-users-in-a-segment-event">inline identification</a> of users in your Segment events.
    </>
  }
/>
