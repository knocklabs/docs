---
title: How to integrate Freshpaint to Knock
description: Learn how to connect your Freshpaint events to Knock to power your workflows
section: Integrations > Sources
layout: integrations
---

import Callout from "../../../components/Callout";

## Getting started

Knock integrates with [Freshpaint](https://freshpaint.com) as a downstream destination to sync customer and event data.

<Callout
  emoji="🚧"
  text={
    <>
      Our Freshpaint integration is currently in beta. If you'd like early
      access, or this is blocking your adoption of Knock, please{" "}
      <a href="mailto:sales@knock.app?subject=Freshpaint integration">
        contact our sales team
      </a>
    </>
  }
/>

## Usecases

You can use our Jitsu integration to:

1. Sync customer data into Knock
2. Trigger workflows from track events
