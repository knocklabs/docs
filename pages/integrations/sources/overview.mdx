...
A source is any platform that can pass event and/or user data into Knock. These can be CDPs (such as [Segment](/integrations/sources/segment) and [Rudderstack](/integrations/sources/rudderstack)) or reverse ETLs (such as Hightouch and Census.) The incoming events from these services can be used to orchestrate actions in Knock such as creating/updating users and triggering workflows.
...
For sources that support identifying users (Like Segment or Rudderstack `identify` calls), each environment configuration for that source includes a setting to enable or disable identifying users. After creating an integration source, enable identifying users for that environment.
