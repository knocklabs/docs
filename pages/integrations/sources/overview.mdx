...
A source is any platform that can pass event and/or user data into Knock. These can be CDPs (such as [Segment](/integrations/sources/segment) and Rudderstack) or reverse ETLs (such as Hightouch and Census.) The incoming events from these services can be used to orchestrate actions in Knock such as creating/updating users and triggering workflows.
