...
<Attributes>
  ...
  <Attribute name="knock.workflow_recipient_run.total" type="count" description="How many workflow recipient runs have been started, segmented by `workflow` key and `exec_mode`." />
  <Attribute name="knock.workflow_recipient_run_error.total" type="count" description="How many errors were experienced during a workflow recipient run, segmented by `workflow` key, `exec_mode`, `step_type`, and the error `reason`. A workflow recipient run can report more than one error." />
