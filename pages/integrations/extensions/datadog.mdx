...
* `knock.message_delivered.total` reports how many messages have been delivered, segmented by `channel`.
* `knock.message_delivered_error.total` reports how many deliveries ended in failure, segmented by `channel`.
* `knock.workflow_recipient_run.total` reports how many workflow recipient runs have been started, segmented by `workflow` key.
* `knock.workflow_recipient_run_error.total` reports how many errors were experienced during a workflow recipient run, segmented by `workflow`. A workflow recipient run can report more than one error.
* `knock.integration_event_received.total` reports the raw number of events received by Knock from an [integration source](https://docs.knock.app/integrations/sources/overview), segmented by `source_type`.
* `knock.integration_action_run.total` reports how many actions were triggered by received events, segmented by `source_type` and `action`.
* `knock.integration_action_run_error.total` reports how many actions failed to run, segmented by `source_type` and `action`.
