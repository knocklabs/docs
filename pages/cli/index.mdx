...
- Synchronize your translation files to Knock.
- Integrate Knock into your CI/CD environment to automatically promote changes.

[Follow the installation guide to set up the Knock CLI](/developer-tools/knock-cli#installing-the-cli).

</ContentColumn>
</Section>

<Section title="Authenticating" slug="authenticating">
<ContentColumn>
...
[Learn more about creating a service token](/developer-tools/service-tokens).

**Setting up a configuration file (optional)**

A service token is required by the CLI for most commands. So, for convenience, Knock CLI supports a user configuration file, where you can store the service token for the CLI to read automatically rather than having to manually pass in with `--service-token` flag for every command.
...
```bash Verifying your service token
...
You can see all your existing workflows in a given environment with the workflow list command. Use a --environment flag to specify the target environment; if omitted, Knock CLI will default to development environment.
...
You can show more details about a given workflow with the `workflow get` command, followed by the target workflow key. Use a `--environment` flag to specify the target environment; if omitted, will default to the development environment.
...
- You can also pass in the `--commit` flag (with an optional `--commit-message` flag) to commit the upserted changes at once.
...
You can validate a brand new or updated workflow directory with the `workflow validate` command. Knock will validate the given workflow payload in the same way as it would with the `workflow push` command, except without persisting those changes.
...
List all translations in the environment. Defaults to listing in development.
...
Pulls one or more translation files from Knock into the local, working directory. When `<locale>` is given only that locales translations are pulled.
...
<Attributes>
  ...
  <Attribute
    name="--all"
    type="boolean"
    description="Should all translation files be pulled? Defaults to true."
  />
</Attributes>
...
<Attributes>
  <Attribute
    name="--all"
    type="boolean"
    description="Should all translation files be pushed? Defaults to false."
  />
  <Attribute
    name="--commit"
    type="boolean"
...
<Attributes>
  <Attribute
    name="--all"
    type="boolean"
    description="Should all translation files be validated? Defaults to false."
  />
</Attributes>
