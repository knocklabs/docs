...
To support this use case within Knock, we can pass a `tenant` identifier into our notify calls. This `tenant` does not have to be configured in any way beforehand, it can simply be a unique identifier you choose to represent this group.
...
You can learn more about how to set per-tenant preferences in [our preferences guide](/send-and-manage-data/preferences#per-tenant-preference-sets). 

<Callout
  emoji="🛣"
  text={
    <>
      <span className="font-bold">Beta feature.</span> Per-tenant preferences are currently in beta. If you're interested in trying this functionality please reach out at <a href="mailto:support@knock.app">support@knock.app</a>. 
    </>
  }
/>
