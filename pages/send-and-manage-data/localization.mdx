...
You can localize your message templates by using the `t` Liquid function helper and translation files you create in Knock. This is useful when you want to send the same notification in different languages, based on your recipient’s locale. This keeps your notifications streamlined - rather than using conditionals within the template, you can reference the text that will be translated right from the template itself.
...
Translations are a JSON object that contain the text for your messages in various locales. For example, let’s say you have a customer order notification you’re building and want to localize it for French and English users. Your translations might look like this:
...
Note: in order to see translations in your email previews, you must commit them to your environment first.
...
Knock supplies a `t` filter for you to reference your translation files from within the message template. As shown in the Basic usage example, you’ll reference the key of the translation you want to use followed by `| t` and it will supply the translation for the recipient’s locale. The `t` filter also allows you to use variables, other filters, and special pluralization rules as well, which we’ll cover here.
...
*Note: at this time, you can’t use filters inside of a translation’s variable. For example, this is not possible:

```HTML
{
  "comment": "New comment from {{ actorName | titlecase }} on your post."
}
```
