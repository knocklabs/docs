...
To schedule a workflow for a recipient, you must first have a valid, committed workflow in your environment. We can then set a schedule for one or more recipients (up to 100 at a time). There is no upper bound on the number of recipients you can set on a given schedule.
