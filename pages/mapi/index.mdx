...
Runs a saved workflow of a given key.

If the `workflow` key is present with a full workflow to run, it will use that instead of looking up the workflow that pertains to the given key. This will operate the same way for workflows that differ from their saved version as for workflow keys that don't yet exist in Knock.
...
<Attributes>
  <Attribute
    name="environment"
    type="string"
    description="The slug of the environment to run this workflow in (defaults to development)."
  />
</Attributes>
<Attributes>
  <Attribute
    name="recipients"
    type="string[]"
    description="A list of recipient ids who will receive the workflow."
  />
</Attributes>
<Attributes>
  <Attribute
...
<Attributes>
  <Attribute
    name="workflow"
    type="Workflow"
    description="An optional workflow object to run."
  />
</Attributes>
...
// Success
{
  "workflowRunId": "aa6f6fb8-d8a7-45d2-aa2a-d593764adc92"
}
