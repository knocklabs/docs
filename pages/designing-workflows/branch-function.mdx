...
The branch function allows you to execute discrete branches of logic within your workflows using our powerful [conditions builder](/concepts/conditions) to specify the criteria for when a branch should or should not execute.
...
For each branch step, a default branch must always exist, although the default branch does not need to contain any steps. When none of the preceding branches evaluate to `true` then the default branch is selected.

<Callout
  emoji="💡"
  text={
    <>
      <strong>Note</strong>: default branches can never have conditions
      associated.
    </>
  }
/>
...
<Callout
  ...
  text={
    <>
      <strong>Note</strong>: at least one branch must continue execution, not
      all branches can terminate.
    </>
  }
/>
...
Yes, absolutely. You can nest delays, throttles, and batches inside of branches as well.
