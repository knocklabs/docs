...
1. **A throttle key** (optional): what should we throttle on?
2. **A throttle window**: how long should the throttle period be?
3. **A throttle threshold**: what’s the total number of invocations that are allowed within the window? Defaults to 1 if none provided.
...
The throttle threshold determines _how many invocations are allowed in the window_ before the threshold takes effect. By default, this value is set to 1, but you can change it as needed.
...
A throttle is allowed to be opened for a maximum of 1 year.
