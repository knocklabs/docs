...
It's important to consider adding security to your fetch requests to secure your endpoint against the outside world. There are currently two options to do this within Knock; using authentication headers, or adding request signing.

### Adding authentication via headers

One option for adding authentication is to use a **shared secret** between Knock and your service's endpoint that you'd inject into the headers of the request. You can use our [secret variables](/send-and-manage-data/variables#setting-secret-variables) to create and store this secret within Knock, ensuring that it can be unique per environment and also obfuscated within the dashboard across all usage.

Variables can be accessed under the `vars` namespace in liquid. So, to add a secret into a header we'd use the syntax `{{ vars.your_variable_name }}` in the header value field.
...
The signature is generated with a HMAC using the SHA256 algorithm and, before being encoded, is comprised of the timestamp and the stringified JSON payload of the request. We encode `"timestamp in numerical form"."stringified payload"` as the signature of the request.
