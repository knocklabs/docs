...
If you need multiple versions of a template based on the data you trigger a notification with, you can make them within the template editor.

You'll notice that when you create a step, you'll be supplied with a template called "Default". This is the one that will send to all recipients when this step is executed. If you want different variants based on a recipient's locale, you can create them by clicking the plus button next to the "Default" tab. You'll be prompted to select a locale, and then your new variant will be created. Then, you can write it in the language that corresponds to that locale.

If your default template variant already has visual elements you'd like to duplicate, you can select "Duplicate variant" from the three-dot menu in the tab header. Keep in mind you'll still have to alter the text to what you'd like it to be, but the entire message contents will be copied to the variant locale you select.

In order for these language variants to be triggered, the recipient must have a key of `locale` that matches the language variant condition. If the recipient's locale does not match any of the configured variants, the default variant will be sent.

You can also edit and delete all template variants except for the default.
