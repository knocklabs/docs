...
- When the batch window interval closes, the workflow continues to the next step, with the data collected in the batch. The data collected in the batch will include the combined workflow trigger data of first (or last) 10 activities from the batch
  (or up to 100 on Enterprise plan) depending on how the batch step is configured. You will find more information about this in the rest of this page.
...
<Callout
  ...
  text={
    <>
      <span className="font-bold">Enterprise plan feature.</span> The render limit setting for batch activities and actors is only available on our <a href="https://knock.app/pricing">Enterprise plan.</a>
    </>
  }
/>
...
On Enterprise plan, you can configure the maximum number of `activities` and `actors` to be rendered in your templates beyond the default limit of 10, to any number between 2 and 100.
...
activities in the batch. On Enterprise plan, On Enterprise plan, you can configure to include up to 100 via the [render limit setting](/designing-workflows/batch-function#setting-the-batch-render-limit).
