...
- When the batch window interval closes, the workflow continues to the next step, with the data collected in the batch. The data collected in the batch will include the combined workflow trigger data of the 10 first (or last) activities from the batch
depending on how the batch step is configured. You will find more information about this in the rest of this page.
...
We will always expose at most 10 activities to your template rendered in your batch (available under the `activities` variable). The `total_activities` will always include the total amount of bundled
activities in the batch.
...
In order to understand how the combined trigger call data will look like, let's take a look at the following example.
Lets consider the case were a message was generated after a batch step with 2 batched activities closes.
