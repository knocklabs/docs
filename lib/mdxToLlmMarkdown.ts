...
const normalizeInlineWhitespace = (input: string): string =>
 ...
 */
const unwrapExpression = (expression: string): string => {
...
const skipQuoted = (source: string, index: number): number => {
  ...
  while (
    ...
    !(source[cursor] === quote && source[cursor - 1] !== "\\")
  ) {
    cursor++;
  }
  return cursor + 1;
 ...
 * (`isRequired`). Hand-rolled character scan rather than a regex so that quotes,
 * backticks, and braces inside a value don't break parsing — that stateful,
 * position-dependent logic doesn't map onto array methods.
 */
const parseJsxAttributes = (input: string): Record<string, AttributeValue> => {
  ...
  while (index < length) {
    ...
    if (valueChar === '"' || valueChar === "'") {
      ...
      if (inner === "`" || inner === '"' || inner === "'") {
        // A single quoted or backtick string wrapping the value.
        index++;
        const valueStart = index;
        while (
          index < length &&
        ...
        const rawValue = input.slice(valueStart, index);
        attributes[name] =
          inner === "`" ? rawValue.replace(/\\`/g, "`") : rawValue;
        index++;
...
const renderAttributeItem = (
  ...
  // Types are shown as inline code, but some already contain backticks (e.g. an
  // enum written as `a` | `b`); wrapping those again would produce broken nested
  ...
  const heading = `- **${name}**${typeSuffix}${requiredSuffix}${descriptionSuffix}`;

  const trimmedChildren = childrenMarkdown.trim();
  if (!trimmedChildren) return heading;
...
/**
 * Finds the end of a JSX opening tag, skipping over quoted/braced attribute
 * values. Hand-rolled scan so a `>` inside a string or expression doesn't get
 * mistaken for the end of the tag.
 */
const findOpeningTagEnd = (
  source: string,
  from: number,
 ...
 *
 * Scans tag by tag rather than using array methods because each match's length
 * (self-closing vs. block form) determines where the next one starts.
 */
const renderAttributeList = (inner: string): string => {
  const items: string[] = [];
  let index = 0;
 ...
 * groups are converted before their parents, keeping each pass free of nesting.
 */
export const convertAttributesToMarkdown = (content: string): string => {
  let result = content;
  ...
  while (openIndex !== -1) {
    ...
    const rendered = renderAttributeList(inner);
 ...
 */
export const resolveTypedocReferences = (
  content: string,
  typedocsDir: string,
): string =>
  content.replace(/<Typedoc\s+file="([^"]+)"\s*\/>/g, (match, file: string) => {
    const typedocPath = path.join(typedocsDir, `${file}.mdx`);
    if (!fs.existsSync(typedocPath)) {
      console.warn(`Warning: Typedoc file not found for LLM markdown: ${file}`);
      return match;
    }
