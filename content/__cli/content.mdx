...
Each resource type has its own directory structure, which is described in detail in the sections below for each resource type. The file-based resources used by the CLI match the schema used by the [Management API](/mapi-reference), so you can use either tool to manage your Knock resources programmatically.
...
If you're migrating your local translation files into Knock, you can arrange them using the file structure above and then push them into Knock with a single command using [`knock translation push --all`](/cli/translation/push). Each `<locale>.json` or `<namespace>.<locale>.json` file should follow the structure defined in the [translation JSON schema](https://schemas.knock.app/cli/translation.json).
...
If you're migrating your local partial files into Knock, you can arrange them using the example file structure above and then push them into Knock with a single command using [`knock partial push --all`](/cli/partial/push). Each `partial.json` file should follow the structure defined in the [partial JSON schema](https://schemas.knock.app/cli/partial.json).
...
If you're migrating your local guide files into Knock, you can arrange them using the example file structure above and then push them into Knock with a single command using [`knock guide push --all`](/cli/guide/push). Each `guide.json` file should follow the structure defined in the [guide JSON schema](https://schemas.knock.app/cli/guide.json).
...
If you're migrating your local message type files into Knock, you can arrange them using the example file structure above. Each `message_type.json` file should follow the structure defined in the [message type JSON schema](https://schemas.knock.app/cli/message_type.json).
