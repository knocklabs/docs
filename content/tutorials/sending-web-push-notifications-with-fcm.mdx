...
<Callout emoji="💡">
  It's possible to store a push token per device for a user, so if you're using
  mobile push as well, it may be better to refetch the browser push token and
  filter it from the `tokens` array before resetting a user's channel data.
</Callout>
