...
description: Learn how to map use FCM and Knock to deliver web push messages to a React application.
section: Tutorials
---

In this tutorial, we'll walk through how to configure Knock and Firebase Cloud Messaging (FCM) to send web push notifications and customize their display using the Firebase SDK. This tutorial assumes you are using a React application and have already implemented other [Knock-specific components](/in-app-ui/react/overview), like the `KnockProvider`.
...
Many of the steps you'll need to take to successfully register for and receive web push messages will happen inside of your application.

The Firebase SDK is responsible for creating a `token` that registers the user's browser and displaying certain types of notifications to the user.
...
You will use both the `app` and `messaging` packages in the following steps.
...
FCM requires a service worker file to handle displaying push notifications. Create a file called [`firebase-messaging-sw.js`](http://firebase-messaging-sw.js) in your `public` directory and include the following contents:
...
The important part of this file is that a new Firebase `app` is initialized and a new instance of `messaging` is created to automatically handle background notifications. Since [service workers](https://developer.mozilla.org/en-US/docs/Web/API/Service_Worker_API) run on their own thread, separate from the JavaScript that powers your app, you'll need to bring Firebase into your application as well and pass a reference to this service worker.

### Create a FirebaseProvider component

In the next step, we'll create a component that initializes the Firebase SDK and registers our service worker, providing both the `messaging` instance and the service worker to other components through [React context](https://react.dev/learn/passing-data-deeply-with-context).
...
The `FirebaseProvider` component is also a great place to centralize your logic for handling foreground messages if you want to use them. In this file, you can see an `onMessage` callback that handles showing foreground notifications using browser APIs. We'll revisit this section of code later when discussing the default behavior of push notifications, e.g. foreground vs. background.
...
For this tutorial, we'll create two UI buttons that subscribe and unsubscribe a user from web push notifications.

### Get a web push token
...
Once the Browser has notifications enabled for this host, we can use the Firebase SDK to request a token specific to this browser. We call the `getToken` method, and pass in the Firebase context `messaging` and `serviceWorkerRegistration` values, as well as the `NEXT_PUBLIC_FIREBASE_VAPID_KEY` as paramters:
...
\<Callout\>  
It's possible to store a push token per device for a user, so if you're using mobile push as well, it may be better to refetch the browser push token and filter it from the `tokens` array before resetting a user's channel data.  
\</Callout\>
...
One of the common differences you'll see in web push messages is [`data` vs. `notification` messages.](https://firebase.google.com/docs/cloud-messaging/concept-options#notifications_and_data_messages)

#### Notification messages

By default, Knock sends `notification` message type payloads to FCM, which already contain a set of pre-defined key/value pairs. All `notification` messages will contain a top-level `notification` key, but may also contain an optional `data` payload.
...
You can also use payload overrides to send [web push platform-specific customization options](https://firebase.google.com/docs/cloud-messaging/concept-options#example-notification-message-with-platform-specific-delivery-options). Using the `webpush` key in your FCM payload, you can override options on the `Notification` browser API or send any other values allowed by [the WebPushConfig object](https://firebase.google.com/docs/reference/fcm/rest/v1/projects.messages#WebpushConfig).
...
<AccordionGroup>
  <Accordion title="Can I use FCM web push messages with any browser?">
    Yes, FCM web push is supported by all major modern browsers.
  </Accordion>
