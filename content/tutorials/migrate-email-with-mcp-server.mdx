    ...
    <>
      ...
      <ul>
        <li>
          ...
          available. In Cursor, you should enable "MAX mode" on your agent. Note
          that this may have billing implications depending on your Cursor plan.
        </li>
        <li>
          ...
          will add to the existing analysis report if it has already been run.
        </li>
        <li>
...
<Steps titleSize="h3">
  <Step title="Analyze your existing email templates">
    ...
    **Prompt:**
    ...
    I need to analyze my existing email templates to prepare for migration to Knock. I have provided access to my HTML email template files. Only reference the files that I've explicitly provided as context.
    ...
    Reference the Knock documentation with the Knock MCP server as needed. The final migrated templates need to be pixel-perfect replicas of the original templates.
    ...
    4. **Implementation plan**: Provide a clear mapping that shows:
      ...
      - Any template-specific HTML content that should be added to each email in Knock
  ...
  <Step title="Test the migrated templates">
    ...
    **Prompt:**
    ...
    1. **Trigger the workflow**: Trigger the workflow with the Knock MCP server, using the test data provided in the summary report. The recipient should be my test user with ID "FILL IN TEST USER ID HERE".
