...
- Hold shared chat credentials and select [tenant-scoped chat connections](/managing-recipients/setting-channel-data#scope-chat-connections-by-tenant), so the same recipient can route to a different destination for each tenant.
- Include tenant IDs as an opt-in tag on [Datadog](/integrations/extensions/datadog) and [New Relic](/integrations/extensions/new-relic) metrics.
...
- Select chat connections tagged with the tenant ID, with untagged connections as a fallback.
...
## Tenant-scoped chat connections

A user or object can store several Slack, Microsoft Teams, or Discord connections and route each tenant's workflow runs to a different destination. Add `knock_tenant_id` to each connection and set it to the corresponding Knock tenant ID. Then pass that tenant ID when you trigger the workflow.

For example, a project object shared by Acme and Globex can store one Slack channel for each company. A workflow triggered with `tenant: "acme"` sends only to connections tagged with `knock_tenant_id: "acme"`. If no connection matches, Knock falls back to untagged connections and never sends to a connection tagged for another tenant.

See [scoping chat connections by tenant](/managing-recipients/setting-channel-data#scope-chat-connections-by-tenant) for setup examples, complete selection behavior, and how tenant-stored Slack and Microsoft Teams credentials work with scoped connections.
...
<AccordionGroup>
  <Accordion title="Where do I find tenants in the dashboard?" anchorSlug="faq-find-tenants-in-dashboard">
  ...
  <Accordion title="What's the limit on the number of tenants I can sync into Knock?" anchorSlug="faq-tenant-sync-limit">
    There are no limits associated with tenants.
  </Accordion>
  <Accordion title="Can I use tenants on a non-enterprise plan?" anchorSlug="faq-tenants-on-non-enterprise-plan">
  ...
  <Accordion title="How does Knock map my user access permissions onto a tenant?" anchorSlug="faq-mapping-user-access-permissions-to-tenant">
  ...
  <Accordion title="Can I use a tenant as a recipient in a workflow trigger?" anchorSlug="faq-tenant-as-workflow-recipient">
  ...
  <Accordion title="Can I subscribe recipients to a tenant?" anchorSlug="faq-subscribe-recipients-to-tenant">
  ...
  <Accordion title="Can I create per-tenant templates?" anchorSlug="faq-per-tenant-templates">
  ...
  <Accordion title="Can I create per-tenant workflows?" anchorSlug="faq-per-tenant-workflows">
  ...
  <Accordion title="Can I nest tenants?" anchorSlug="faq-nested-tenants">
