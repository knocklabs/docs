...
For the default preference set, use `knockClient.user.getPreferences()`. Per-tenant preferences live in a separate preference set, so pass the tenant id as the `preferenceSet` parameter on `knockClient.user.getPreferences()`.
