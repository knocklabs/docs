...
<AccordionGroup>
  <Accordion title="Can I replace a tenant's full default preference set instead of merging my changes with any existing preferences?" anchorSlug="faq-replace-tenant-default-preferences">
  ...
  <Accordion title="How can I remove a tenant's default preference set after it has been set?" anchorSlug="faq-remove-tenant-default-preferences">
  ...
  <Accordion title="How can I force a tenant's default preferences to override a user's per-tenant preferences during preference evaluation?" anchorSlug="faq-tenant-defaults-override-user-preferences">
  ...
  <Accordion title="How can I opt all recipients under a tenant out of commercial messages, even when they are individually opted in?" anchorSlug="faq-tenant-wide-commercial-opt-out">
    ...
    text={
      <>
        To comply with CAN-SPAM regulations, you should never use preference conditions to opt a recipient in to commercial messages under a given tenant when they have explicitly opted out under their own settings. Always set a <code>false</code> value on a user's preferences when they opt out of commercial messages.
      </>
    }
    />
