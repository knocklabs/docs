...
<AccordionGroup>
  ...
  <Accordion title="How can I remove a tenant's default preference set after it has been set?">
  ...
  <Accordion title="How can I opt all recipients under a tenant out of commercial messages, even when they are individually opted in?">
    A `commercial_subscribed` value on a recipient's `default` preference set always takes precedence over tenant and environment defaults. In a multi-tenant application, you may want to give tenant admins a switch that turns off [commercial messages](/preferences/commercial-unsubscribe) for everyone in their tenant, even for users who are individually opted in. 
    
    You can model this by storing an individual user's opt-in as a condition on the recipient's `commercial_subscribed` preference that reads the tenant's own setting at send time, instead of an explicit `true` setting:
    ...
    Then, set the relevant tenant's `commercial_subscribed` preference to `false`:
    ...
    <Callout type="alert"
    title="Do not use the reverse approach."
    text={
      <>
        To comply with CAN-SPAM regulations, you should never use preference conditions to opt a recipient in to commercial messages under a given tenant when they have explicitly opted out of receiving commercial messages under their own preferences.
      </>
    }
    />
