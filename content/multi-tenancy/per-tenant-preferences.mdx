...
The examples below use the server-side Node SDK with a secret API key. To manage a user's own per-tenant preferences from your frontend, use a public API key with a signed user token and pass the tenant ID as the `preferenceSet`. See [Build your preference center](/preferences/build-your-preference-center) for details.
