...
For most resources, cloning is a dashboard-only feature and is not available through the [Knock CLI](/cli/overview) or [Management API](/developer-tools/management-api). [Goals](/concepts/goals) are the exception: you can also clone them with the Management API.
