...
To version the changes you make in your [environments](/version-control/environments), Knock uses a commit model. When you make a change to a workflow or a layout in the Knock dashboard, you'll need to commit it to your development environment before those changes will appear in workflows triggered via the API. By default, changes must be made in development and promoted to production. If you've enabled [production write access](/version-control/environments#production-write-access), resources can also be edited directly in production.
...
<Callout
  ...
  text={
    <>
      By default, all changes to production must be promoted from development.
      If you've enabled{" "}
      <a href="/version-control/environments#production-write-access">
        production write access
      </a>
      , you can choose to edit resources directly in production instead of using
      promotion. Promotion remains available and is still the recommended
      approach for version-controlled workflows.
    </>
  }
/>
...
<Callout
  type="warning"
  title="Origin cannot be changed"
  text="A resource's origin environment is permanent and cannot be changed. If you need to move a workflow from version control to direct production editing (or vice versa), you must create a new workflow in the desired environment."
/>
