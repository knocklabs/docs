...
<figure className="text-center">
  <Image
    src="/images/version-control/branch-selector.png"
    width={400}
    height={282}
    className="mx-auto"
    alt="The branch selector in the Knock dashboard."
  />
  <figcaption>The branch selector in the Knock dashboard.</figcaption>
</figure>
...
<Callout
  type="warning"
  text="You cannot modify an audience on a branch. To add or remove users from an audience on a branch, you must do so on the main branch."
/>
...
- [Goals](/concepts/goals) cannot be created, updated, or attached on a branch.
...
<AccordionGroup>
  <Accordion title="What is the difference between a branch and an environment?" anchorSlug="faq-branch-vs-environment">
    A branch is a way to isolate changes to your Knock resources. It's like a
    sandbox for your changes.
  </Accordion>
  <Accordion title="What channel settings are used for a branch?" anchorSlug="faq-branch-channel-settings">
    Branches will always use the channel settings configured for the `development` environment (the `main` branch).
  </Accordion>
  <Accordion title="Can I block changes being made directly to my development environment?" anchorSlug="faq-block-direct-development-changes">
  ...
  <Accordion title="Can I put an approval process in place before merging changes from a branch into the main branch?" anchorSlug="faq-branch-merge-approval-process">
    No, there's not currently a way to put a review process in place before
    merging changes from a branch into the main branch.
  </Accordion>
  <Accordion title="Is there a limit to the number of branches I can have in my account?" anchorSlug="faq-branch-limit-per-account">
