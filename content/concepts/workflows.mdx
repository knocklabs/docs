...
Every workflow can be [triggered directly via an API call](/send-notifications/triggering-workflows/api) to the workflow trigger endpoint. In addition to API triggering, you can configure a workflow with one of these trigger types:
...
- **Audiences**: workflows can be [triggered when a user joins an audience](/send-notifications/triggering-workflows/audiences), enabling lifecycle messaging based on user segments.

Configuring an event, schedule, or audience trigger adds an additional way to trigger the workflow — it does not disable API triggering.
