...
The **Analytics** tab on a workflow reports how its runs progress through each step and how its messages are delivered and engaged with. See [workflow analytics](/send-notifications/analytics#workflow-analytics) for details.
...
<AccordionGroup>
  <Accordion title="Is there a limit to the number of workflows I can have in Knock?" anchorSlug="faq-workflow-limit">
    No, there's no limit to the number of workflows you can have within your
    Knock environment.
  </Accordion>
  <Accordion title="Can I build a single workflow for all of my notification use cases?" anchorSlug="faq-single-workflow-for-all-notifications">
  ...
  <Accordion title="Can I create per-tenant/per-customer workflows?" anchorSlug="faq-per-tenant-workflows">
  ...
  <Accordion title="Is there a way to disable a workflow without archiving it?" anchorSlug="faq-disable-workflow-without-archiving">
  ...
  <Accordion title="What happens to in-progress workflow runs when I set a workflow's status to Inactive?" anchorSlug="faq-inactive-workflow-in-progress-runs">
