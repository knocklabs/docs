...
##### Understanding "from now" vs "ago"

When using relative timestamps, the modifier determines which direction in time to compare:

- **"from now"** creates a point in the future. Use this for conditions about upcoming events.
- **"ago"** creates a point in the past. Use this for conditions about historical events.

Here are some common patterns:

| Use case                                     | Operator                   | Argument             | Example                                                                      |
| -------------------------------------------- | -------------------------- | -------------------- | ---------------------------------------------------------------------------- |
| Users who signed up more than 30 days ago    | `is_timestamp_before`      | 30 days **ago**      | `recipient.created_at` is before the point 30 days in the past               |
| Orders placed in the last 7 days             | `is_timestamp_on_or_after` | 7 days **ago**       | `data.order_placed_at` is on or after the point 7 days in the past           |
| Subscriptions expiring in the next 3 days    | `is_timestamp_before`      | 3 days **from now**  | `recipient.subscription_expires_at` is before the point 3 days in the future |
| Appointments scheduled more than 2 weeks out | `is_timestamp_on_or_after` | 2 weeks **from now** | `data.appointment_at` is on or after the point 2 weeks in the future         |

##### Timestamp condition limitations

- **Minimum duration.** Relative timestamps must be at least 15 minutes.
- **Precise calculations.** Relative times are calculated precisely from the current time. For example, "1 day from now" means exactly 24 hours from the moment the condition is evaluated, not "tomorrow".
- **Between operator.** The `is_timestamp_between` operator only supports absolute dates or dynamic variables, not relative timestamps.
...
| `not_contains_all`         | are all `argument` not in `variable` (works with single arguments, or lists of arguments) |
| `empty`                    | `variable` in `["", null, []]`                                                            |
| `not_empty`                | `variable` not in `["", null, []]`                                                        |
| `is_timestamp_before`      | Checks if the variable is a timestamp before the argument value                           |
| `is_timestamp_on_or_after` | Checks if the variable is a timestamp on or after the argument value                      |
| `is_timestamp_between`     | Checks if the variable is a timestamp between the argument's start and end values         |
