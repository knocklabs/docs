...
<Callout
  ...
  text={
    <>
      ...
      <ul>
        <li>
          <strong>Minimum duration.</strong> Relative timestamps must be at least 15 minutes.
        </li>
        <li>
          <strong>Precise calculations.</strong> Relative times are calculated precisely from the current time. For example, "1 day from now" means exactly 24 hours from the moment the condition is evaluated, not "tomorrow".
        </li>
        <li>
          <strong>Between operator.</strong> The <code>is_timestamp_between</code> operator only supports absolute dates or dynamic variables, not relative timestamps.
        </li>
      </ul>
    </>
  }
