...
Workflows are triggered journeys that send notifications to your recipients. Every workflow can be triggered via an API call. Workflows can also be configured to trigger via an event, on a schedule for a recipient, or when a user enters a specific audience. Workflows consist of channel and function steps.
