...
When you save a goal, Knock publishes it. Goals do not use the commit-and-promote model that workflows and other versioned resources use.

### Versioning

Changes to a goal's title, description, or condition create a new goal version. Attachments reference the goal by **key**, not a pinned version. New completions use the latest published definition. Completions that already happened stay associated with the version that was live when they occurred. On the goal overview, you can break reporting down by goal version.
...
If a workflow, broadcast, or guide still has the goal attached, detach it before you archive.
...
| **Recipient updated**     | A [user](/concepts/users) property changes. Users only; not [objects](/concepts/objects).                  | Required. Filter on recipient properties under `recipient.*`. |
...
- **Audience entry / exit.** The [audience](/concepts/audiences) exists in the same environment, and the recipient entered or exited it.
- **Recipient updated.** You [identified](/managing-recipients/identifying-recipients) the recipient as a user and updated a property that matches the condition. Recipient-updated goals do not complete for objects.

### Example: recipient property updated

The following examples show the `condition` payload used by the [Management API](/mapi-reference/goals). In the dashboard, pick the event type and match conditions in the goal editor.
...
- Recipient-updated conditions for [objects](/concepts/objects). Recipient-updated goals apply to users only.
