...
- Is uniquely identified by an `id`, [per-environment](/concepts/environments). In most cases, this `id` is the same `uuid` used to identify the tenant in your system
...
<Callout
  ...
  text={
    <>
      ...
      <a href="/concepts/objects">Object</a> in a special-system
...
It's important to note that tenants **do not** have a relationship to the [users](/concepts/users) and [objects](/concepts/objects) you've identified in Knock. That means Knock does not know _which tenant_ to associate with the set of users you're triggering a notification for. Instead, you must explicitly tell Knock as part of a workflow trigger to associate the workflow runs with a tenant.
...
When a workflow is triggered with a `tenant` property, all of the [Messages](/concepts/messages) produced in the workflow run will be tagged with the `id` of the tenant.
...
You can learn more about how to set per-tenant preferences and tenant preference defaults in [our preferences guide](/concepts/preferences#per-tenant-preference-sets).
