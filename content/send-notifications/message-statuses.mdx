...
- **In-app** — [Knock link tracking](/send-notifications/tracking) needs to be enabled for link clicks to count towards this status. Only links within the message body will be tracked; clicks on the [action URL](/integrations/in-app/feeds#action-url) that can be configured on the notification cell are tracked as [`interacted` events](#interacted). (**Note:** clicking “mark all as read” does not result in a message being marked as clicked; rather, as the phrasing implies, we bulk update the message engagement statuses to opened/read.)
...
Knock also records a [`message.complaint`](/developer-tools/outbound-webhooks/event-types#messagecomplaint) event when a recipient reports a delivered email as spam. A complaint is a post-delivery event and does not change the message's delivery or engagement status. Complaint events are available on email providers with delivery status webhooks configured to send complaint events. See your provider's [integration page](/integrations/email/overview) for setup instructions.
