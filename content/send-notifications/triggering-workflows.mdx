...
In some situations, you may want to limit the number of times that a recipient can run through a workflow. For example, for an account sign up workflow, you may want to ensure that a recipient can only ever run through the workflow once. That's where workflow trigger frequency comes in.
