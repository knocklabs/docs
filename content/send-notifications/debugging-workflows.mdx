...
- **A dynamic [batch](/designing-workflows/batch-function), [delay](/designing-workflows/delay-function), or [throttle](/designing-workflows/throttle-function) step encounters an invalid timestamp.** If a workflow step receives an invalid dynamic window value (like a timestamp in the past), the step is skipped.
...
- **Fetch step errors.** Errors from the HTTP request made by the fetch step.
