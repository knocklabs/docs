...
You can access the workflow debugger from the "Runs" tab on a given workflow or the "Workflow runs" tab on a recipient record. You can also access it under <a href="https://dashboard.knock.app/~/runs" target="_blank">**Observability** > **Runs**</a> in the dashboard sidebar, which lists every workflow run in the current environment. Filter that list if desired, then select a run to open it in the debugger.

Finally, you can reach the debugger under <a href="https://dashboard.knock.app/~/logs" target="_blank">**Observability** > **API logs**</a> in the dashboard sidebar. From there:
