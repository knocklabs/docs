...
<AccordionGroup>
  <Accordion
    title="Can I still trigger a workflow via API if it has an event trigger configured?"
    anchorSlug="faq-api-trigger-with-event-trigger"
  >
  ...
  <Accordion
    title="Can I trigger a workflow for multiple recipients from a single event?"
    anchorSlug="faq-multiple-recipients-from-single-event"
  >
  ...
  <Accordion
    title="Can I cancel a workflow run triggered by an event?"
    anchorSlug="faq-cancel-event-triggered-workflow-run"
  >
