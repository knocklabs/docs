...
From the Trigger step sidebar, if you have events connected to Knock you'll see the option to set the trigger type to "Event" from the dropdown menu. Once here, you can select an event that will trigger this workflow when it's received. You can also map the critical fields needed to run a workflow to the fields that will be in the incoming event payload.

<Callout
  type="info"
  text={
    <>
      Setting the trigger type to "Event" does not disable API triggering. You
      can still trigger the workflow via the{" "}
      <a href="/send-notifications/triggering-workflows/api">
...
<AccordionGroup>
  ...
  <Accordion title="Can I still trigger an event-triggered workflow via the API?">
    Yes. Configuring an event trigger adds an additional way to trigger the
    workflow. You can always trigger the workflow via the [trigger API
    endpoint](/send-notifications/triggering-workflows/api) regardless of the
