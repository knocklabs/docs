...
You can include an optional `Idempotency-Key` header in your trigger call to safely retry requests without creating duplicate workflow runs. If you retry a request with the same idempotency key within 24 hours from the original request, Knock will return the same response as the original request.
