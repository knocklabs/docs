...
Given their brevity and consistent length, Knock uses short links by default for channels that often have character constraints:

- All SMS channels
- WhatsApp chat channel

#### Short links for chat channels

For other chat channels, such as Slack, Microsoft Teams, and Discord, short links are opt-in. To enable them, turn on **Enable chat link tracking** in the channel configuration, then turn on **Use short links for tracked URLs**.
...
<AccordionGroup>
  <Accordion title="My channel provider supports link or engagement tracking natively. What are the advantages of using Knock tracking?" anchorSlug="faq-knock-tracking-vs-provider-tracking">
  ...
  <Accordion title="Can my provider report engagement metrics back to Knock?" anchorSlug="faq-provider-engagement-metrics-to-knock">
