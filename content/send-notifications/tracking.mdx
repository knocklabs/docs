...
If you use Knock's outbound webhooks, you can hook into the `message.read` and `message.link_clicked` events captured via Knock tracking. See the [outbound webhooks guide](/developer-tools/outbound-webhooks) for more details.
...
<AccordionGroup>
  <Accordion title="My channel provider supports link or engagement tracking natively. What are the advantages of using Knock tracking?">
    ...
    - **Capture events via webhooks.** When Knock tracking is enabled, you can power other event-based flows in your product with Knock's [outbound webhooks](/developer-tools/outbound-webhooks).
