...
Knock is listed as a community connector in Anthropic's <a href="https://claude.ai/directory/connectors/knock" target="_blank" rel="noopener noreferrer">Connectors Directory</a>. Use the connector in Claude Cowork and Claude Desktop. Connectors talk to the remote MCP server over HTTPS, so you do not need Node.js or a local proxy.
...
After OAuth completes, Knock tools are available in Cowork and Claude Desktop. From the chat **+** menu, open **Connectors** and make sure Knock is enabled for that conversation.
