...
We've added setup instructions below for Claude, Claude Code, and Cursor, but the same instructions apply to any other MCP client-compatible application.

### Claude

Knock is listed as a community connector in Anthropic's <a href="https://claude.ai/directory/connectors/knock" target="_blank" rel="noopener noreferrer">Connectors Directory</a>. Use it on Claude (web), Claude Desktop, and Claude mobile. Connectors talk to the remote MCP server over HTTPS, so you do not need Node.js or a local proxy.
...
1. Open the <a href="https://claude.ai/directory/connectors/knock" target="_blank" rel="noopener noreferrer">Knock connector</a> in Claude, or browse **Customize** > **Connectors** and search for Knock.
2. Click **Connect** (or **Install**).
3. Approve the MCP access request in Knock.
4. Select the Knock account to connect, then click **Allow access**.
5. Choose which [capabilities](#capabilities) to grant (the defaults are a good starting point), then click **Authorize**.

After OAuth completes, Knock tools are available in your Claude conversations. From the chat **+** menu, open **Connectors** and make sure Knock is enabled for that conversation.

<Callout
  type="info"
  title="Team and Enterprise plans"
  text="On Claude Team and Enterprise, an Owner must first enable the connector under Organization settings > Connectors before members can connect and authenticate individually."
/>

#### Add Knock as a custom connector

If you prefer not to use the directory listing, you can add Knock manually:

1. In Claude, go to **Customize** > **Connectors**.
2. Click the **+** button next to Connectors, then select **Add custom connector**.
3. Enter a name (for example, `Knock`) and the MCP server URL: `https://mcp.knock.app/mcp`.
4. Click **Add**, then click **Connect** and complete the same OAuth flow above.
...
If you prefer a local MCP config in Claude Desktop instead of Connectors, you can use `mcp-remote` as a proxy. This requires Node.js 18 or higher.
