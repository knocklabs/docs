...
Use Knock as a <a href="https://support.claude.com/en/articles/11176164-use-connectors-to-extend-claude-s-capabilities" target="_blank" rel="noopener noreferrer">Claude connector</a> on Claude (web), Claude Desktop, and Claude mobile. Connectors talk to the remote MCP server over HTTPS, so you do not need Node.js or a local proxy.
...
4. Click **Add**, then click **Connect** on the Knock connector.
5. Approve the MCP access request in Knock.
6. Select the Knock account to connect, then click **Allow access**.
7. Choose which [capabilities](#capabilities) to grant (the defaults are a good starting point), then click **Authorize**.
...
<Callout
  ...
  text="On Claude Team and Enterprise, an Owner must first add the connector under Organization settings > Connectors before members can connect and authenticate individually."
/>
