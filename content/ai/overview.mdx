...
**Recommended for external clients and your own apps.** The [MCP server](/ai/mcp-server) at `mcp.knock.app/mcp` exposes Knock primitives to LLMs and AI agents via the Model Context Protocol. Reach for the MCP server when you need Knock tools available inside Claude (including the <a href="https://claude.ai/directory/connectors/knock" target="_blank" rel="noopener noreferrer">Claude connector</a> for Cowork and Desktop), ChatGPT, Cursor, or any MCP-compatible client, or when you're building a product that needs Knock primitives behind an LLM.
