...
<Box marginBottom="20">
  <KnockAiBanner subheadingProps={{ style: { width: "70%" } }} />
</Box>
...
**Recommended for external clients and your own apps.** The [MCP server](/ai/mcp-server) at `mcp.knock.app/mcp` exposes Knock primitives to LLMs and AI agents via the Model Context Protocol. Reach for the MCP server when you need Knock tools available inside Claude (including the <a href="https://claude.ai/directory/connectors/knock" target="_blank" rel="noopener noreferrer">Claude connector</a>), ChatGPT, Cursor, or any MCP-compatible client, or when you're building a product that needs Knock primitives behind an LLM.
...
- **Knock's open-source package** at <a href="https://github.com/knocklabs/skills" target="_blank" rel="noopener noreferrer">github.com/knocklabs/skills</a>, installed into AI coding agents like Cursor or Claude. Includes `knock-cli` (pairs with the [CLI](/ai/cli)) and `notification-best-practices` (pairs with any surface).
...
| Are you using Claude, ChatGPT, or another external client?      | [MCP server](/ai/mcp-server) |
