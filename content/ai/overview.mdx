...
description: Use AI across Knock — in the dashboard, inside workflows, in your IDE, and in external clients.
section: AI
---

Knock exposes AI in four places, organized by where the AI runs: in the Knock dashboard, inside a workflow run, in your IDE alongside a coding agent, and in external clients or your own apps. Use this page to understand each primitive and which one to reach for.
...
## Use AI in the dashboard: Knock agent
...
## Use AI in workflows: agent function
...
## Use AI when building your Knock integration

When you're working in your codebase or wiring AI into your own product, Knock gives you two primitives that are designed to be used together: skills for IDE-based local development, and the MCP server for external clients and applications.

### Skills (recommended for IDE-based local development)
...
Knock publishes an open-source skills package at [github.com/knocklabs/skills](https://github.com/knocklabs/skills).

### MCP server (recommended for external clients and your own apps)

The [MCP server](/ai/mcp-server) at `mcp.knock.app/mcp` exposes Knock primitives to LLMs and AI agents via the Model Context Protocol. Reach for the MCP server when you need Knock as tools inside Claude Desktop, ChatGPT, or any MCP-compatible client, or when you're building a product that needs Knock primitives behind an LLM.
...
## MCP server vs. skills: when to use which
