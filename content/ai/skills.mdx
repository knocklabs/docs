...
The `knock-email-template-migration` skill migrates a directory of existing email templates into Knock. The result is an editable email design system rather than a one-to-one HTML copy: shared headers and footers become [email layouts](/integrations/email/layouts), repeated components become [partials](/template-editor/partials) that teammates can drag into the visual editor, and each email is delivered by a [workflow](/concepts/workflows) with a documented trigger payload.

**Complements.** The [Knock CLI](/ai/cli), which the skill uses to build and push resources, and the [MCP server](/ai/mcp-server), which it uses to render previews during verification. Install `knock-cli` alongside it. See the [migrate email templates tutorial](/tutorials/migrate-email-with-ai-agent) for a full walkthrough.

Use it when you want an agent to:

- Move a set of email templates out of your codebase or an existing provider and into Knock.
- Extract reusable layouts and partials from your templates.
- Map template variables and conditional logic onto a Knock trigger payload contract.
...
| `knock-email-template-migration`    | The [CLI](/ai/cli) and [MCP server](/ai/mcp-server)      | How to turn existing email templates into layouts, partials, and workflows. |
