...
The CLI exposes the full set of Knock resource operations. The [MCP server](/ai/knock-mcp-server) exposes a curated subset focused on managing resources, manipulating data, and inspecting environments — it does not currently support local file scaffolding, local validation, or deletion. Use the CLI when you need any of those.
