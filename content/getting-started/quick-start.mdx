  ...
  <Step title="Create a workflow in Knock">
    ...
    <AccordionGroup>
      ...
      <Accordion title="Design the notification message" anchorSlug="design-the-notification-message">
        After adding a channel step, we can configure the notification's content by clicking on "Edit template" in the channel's edit step view to see that step's [message template](/template-editor/overview). The template starts with default copy, so we'll just use that for now.
      </Accordion>
