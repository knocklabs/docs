...
Every workflow can be triggered via a direct API call. Workflows can also be configured to trigger from a source event in a CDP like Segment, on a recurring or one-off schedule, or automatically when a user joins an audience.
