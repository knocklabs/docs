...
There is a lot more to learn about Knock, and our [concepts overview page](/concepts) is a good place to start. Here are use cases our customers commonly solve with Knock:

- Powering [translation and localization](/concepts/overview#translations) and managing timezone-aware delivery
- Creating advanced notification logic using [subscriptions](/concepts/overview#subscriptions) and [schedules](/concepts/overview#schedules)
- Integrating Knock with your application's data model, using [tenants](/concepts/overview#tenants) and [objects](/concepts/overview#objects) to power customized experiences
...
Knock is a developer-first platform, with both [environment](/concepts/overview#environments) and [commit models](/concepts/overview#commits). If you want to work with Knock resources in code, you can use our [Management API](/developer-tools/management-api) or [CLI](/developer-tools/knock-cli).
