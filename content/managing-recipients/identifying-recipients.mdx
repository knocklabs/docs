...
| `timezone`     | A valid <a href="https://en.wikipedia.org/wiki/List_of_tz_database_time_zones" target="\_blank" rel="noopener noreferrer" style={{ color: "var(--tgph-accent-11)", textDecoration: "underline" }}>tz database time zone string</a> used for [send windows](/designing-workflows/send-windows) and [schedules](/concepts/schedules). |
...
<AccordionGroup>
  <Accordion title="Why isn't inline identification working?" anchorSlug="faq-inline-identification-not-working">
    ...
    ```
      ...
      <Accordion title="Can I set a user's preferences during identification?" anchorSlug="faq-preferences-during-identification">
...
To set preferences during identification, provide a dictionary under the `preferences` key with each key representing a preference set ID. Unlike the preferences [endpoints](/preferences/overview), setting preferences during identification requires explicitly providing the `default` preference set key when updating default preferences.
  ...
  <Accordion title="Can I set channel data during identification?" anchorSlug="faq-channel-data-during-identification">
