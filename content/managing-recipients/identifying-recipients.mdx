...
- **Direct identification**. Make a single server-side API request (`users.identify`) to upsert one recipient.
- **Bulk identification**. Make a single server-side API request (`users.bulkIdentify`) to upsert many recipients at once.
- **Inline identification**. Upsert recipients by including recipient data in other client-side or server-side API requests (e.g., `workflows.trigger`).
...
Make a server-side `users.identify` request to upsert a single recipient in Knock. This is useful to ensure that user data is updated on an ongoing basis, like reflecting updates a user makes to their profile information inside 
of your product.
