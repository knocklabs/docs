    ...
    const YourAppLayout = ({ user }) => {
      return (
        <KnockProvider
          ...
          user={{ id: user.id }} 
        >
          {/* Your app content with Knock components */}
        </KnockProvider>
      );
    };
    ```
