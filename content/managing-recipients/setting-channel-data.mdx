...
At Knock we call this concept <a href="/reference#channel-data"><code>ChannelData</code></a>. `ChannelData` lives under a [user](/concepts/users) or an [object](/concepts/objects) and stores channel-specific data to be used when that user or object is included as a recipient on a [triggered workflow](/send-notifications/triggering-workflows).
...
<AccordionGroup>
  ...
  <Accordion title ="Microsoft Teams">
    ...
    An `MsTeamsConnection` has the following schema:
