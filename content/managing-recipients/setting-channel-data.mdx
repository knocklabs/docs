...
<AccordionGroup>
  ...
  <Accordion title ="Microsoft Teams">
    ...
    An `MsTeamsConnection` can have one of two schemas, depending on whether you're using Microsoft's Bot Framework or an incoming webhook.

    If you're using the Bot Framework, your `MsTeamsConnection` schema looks like this. You'll use either an
    ...
    | Property             | Type     | Description                       |
    | -------------------- | -------- | --------------------------------- |
    | incoming_webhook.url | `string` | The MS Teams incoming webhook URL (to be used instead of the properties above) |
