...
```bash Install the Knock CLI with homebrew
brew install knocklabs/tap/knock
```

<div style={{ marginTop: 80 }} />

```bash Install the Knock CLI with npm
...
```bash Pulling all workflows into ./workflows directory
...
```bash Pushing all workflows from ./workflows directory
...
```bash Validate a single email layout
...
```bash Get translation without a namespace
knock translation get en
```

```bash Get translation with a namespace
knock translation get admin.en
```

```bash Get translation in a different environment
