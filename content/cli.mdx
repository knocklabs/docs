...
```bash title="Login to your Knock account"
knock login
```
...
You can login to your Knock account with the `login` command. This will open a browser window where you can sign in to your Knock account and authorize the CLI to access your account.
...
You can logout of your Knock account with the `logout` command. This will clear the authentication token for the CLI.

</ContentColumn>
<ExampleColumn>

```bash title="Logout of your Knock account"
knock logout
```
...
<Section title="knock guides list [flags]" path="/guides/list">
<ContentColumn>
...
**Note:** Guides are currently in beta.
...
```bash title="Pagination example"
knock guides list --after=xxx
```

</ExampleColumn>
</Section>

<Section title="knock guides get [flags]" path="/guides/get">
<ContentColumn>

You can show more details about a given guide with the `guides get` command, followed by the target guide key.
...
```bash title="Basic usage"
knock guides get my-guide
```

```bash title="Get guide in a different environment"
knock guides get my-guide --environment=production
```

</ExampleColumn>
</Section>

<Section title="knock guides pull [flags]" path="/guides/pull">
<ContentColumn>

You can pull and download guides from Knock to a local file system with the `guides pull` command. Knock CLI will create a new guide directory or update the existing guide directory in the local file system.
...
<Attributes>
  ...
  <Attribute
    ...
    description={
      <>
        <p>
          Whether to pull all guides from the specified environment into the
          target directory path set by `--guides-dir`. Defaults to false.
        </p>
        <p>
          When used, all contents in the target directory will be erased and
          replaced with all guides from the specified environment.
        </p>
      </>
    }
  />
...
```bash title="Basic usage"
knock guides pull my-guide
```

```bash title="Pulling a guide in a different environment"
knock guides pull my-guide --environment=production
```

```bash title="Pulling all guides into ./guides directory"
knock guides pull --all --guides-dir=./guides
```

</ExampleColumn>
</Section>

<Section title="knock guides push [flags]" path="/guides/push">
<ContentColumn>

You can push and upload a guide directory to Knock with the `guides push` command. Knock will update an existing guide by the matching guide key, or create a new guide if it does not exist yet.

Note:

- The `guides push` command only pushes guides into the `development` environment.
- You must be directly above the target guide directory when running the `guides push` command, so the CLI can locate the `guide.json` file.
- You can also pass in the `--commit` flag (with an optional `--commit-message` flag) to commit the upserted changes immediately.
...
knock guides push --all --guides-dir=./guides
```

</ExampleColumn>
</Section>

<Section title="knock guides validate [flags]" path="/guides/validate">
<ContentColumn>

You can validate a new or updated guide directory with the `guides validate` command. Knock will validate the given guide payload in the same way as it would with the `guides push` command, except without persisting those changes.
...
```bash title="Basic usage"
knock guides validate my-guide
```

</ExampleColumn>
</Section>

<Section title="knock guides activate [flags]" path="/guides/activate">
<ContentColumn>

You can activate or deactivate a guide in a given environment with the `guides activate` command. You can either set the active status immediately or schedule it.
...
```bash title="Basic usage"
knock guides activate my-guide \
  --environment=development
```
