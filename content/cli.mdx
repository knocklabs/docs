...
You can activate or deactivate a guide in a given environment with the `guides activate` command.

Note:

- This immediately enables or disables a guide in a given environment without needing to go through environment promotion.
- By default, this command activates a given guide. Pass in the `--status` flag with `false` in order to deactivate it.
