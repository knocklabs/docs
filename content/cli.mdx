...
There is no required directory structure when working with Knock resources locally. However, if you use `knock pull --all` or `knock push --all`, this is the expected directory structure. For forward compatibility, we recommend using this structure to ensure your local files work seamlessly with future CLI updates.
...
Resources will be grouped by resource type within subdirectories of the target directory path set by the `--knock-dir` flag. See the [Directory structure](/cli#directory-structure) section for details on the recommended directory structure.
