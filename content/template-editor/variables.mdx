...
| `cancellation_key`     | The cancellation key of the current workflow run, or `null` if no key was supplied          |

Use `{{ workflow.cancellation_key }}` in a [trigger workflow step](/designing-workflows/trigger-workflow-function#inheriting-the-parent-cancellation-key) to pass the key to a nested workflow.
