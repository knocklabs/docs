...
```liquid title="Message template"
<p> {% t %}Congratulations, {{ recipient.name }}!{% endt %} </p>
```
...
<Callout
  ...
  text={
    <>
      unlike the <code>t</code> filter, the <code>t</code> tag doesn't accept
...
You can learn more about automating translation management in the [Knock CLI reference](/cli/overview). Feel free to <a href="mailto:support@knock.app">contact us</a> if you have questions.
...
<AccordionGroup>
  <Accordion title="Can I use markdown formatting inside my translations?" anchorSlug="faq-markdown-in-translations">
