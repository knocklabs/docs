...
- A simple interface for creating and managing cross-channel message templates.
...
- A test runner for sending test messages to your template.
...
Each [channel step](/designing-workflows/channel-step) you add to a workflow or broadcast will have its own message template. This template is what determines what the message sent to your user look like for a given channel.
...
<Callout
  ...
  text={
    <>
      There is no concept of a standalone template in Knock. All templates must belong to a single channel step within a workflow or broadcast.
    </>
  }
/>
...
From within a channel step, you can access the template editor by clicking the "Edit content" button within the selected step's pane. This will open the template editor for the step, where you can design the contents of the message that will be sent to the recipient.
...
While templates can only be managed within the context of a single channel step, if you want to share content across multiple channels, or multiple templates within the same channel, you can use [partials](/template-editor/partials).

Partials are reusable pieces of content you can use across any of your templates, including email templates. They can include liquid for rich templating, and can even be used as blocks within the visual email editor.
