...
<Attributes>
  ...
  <Attribute
    name="schema"
    type="object"
    description="A <a href=\"https://json-schema.org/\" target=\"_blank\" rel=\"noopener noreferrer\">JSON Schema</a> object that validates the expected structure of the input. Supports `properties` to define the expected keys and their types, and `required` to specify which keys must be present."
  />
</Attributes>
