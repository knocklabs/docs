...
2. A user ID and an auth token
  - Note: Auth tokens are strongly recommended for production environments and required when enhanced security mode is enabled. For more information, see our [Security & Authentication documentation](https://docs.knock.app/in-app-ui/security-and-authentication#authentication-with-enhanced-security).
