...
In the step editor you can edit properties as a JSON blob or in key-value form. Updated properties are stored in Knock and are available to subsequent workflow steps and to the Management API. The updated user is immediately available to the next step and all later steps in the same workflow run.

## Limitations

Update user steps do not support updating [channel data](/managing-recipients/setting-channel-data). You cannot set or change channel data (for example, email or push tokens) via this step.
