...
By default, the agent returns a single string response available as `data.<step_ref>.text`.
...
<AccordionGroup>
  <Accordion title="Can I use the agent output in step conditions?" anchorSlug="faq-agent-output-in-step-conditions">
  ...
  <Accordion title="Does the agent work with batch functions?" anchorSlug="faq-agent-with-batch-functions">
  ...
  <Accordion title="What happens if my prompt renders to invalid Liquid?" anchorSlug="faq-invalid-liquid-prompt">
