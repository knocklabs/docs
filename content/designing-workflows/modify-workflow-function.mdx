---
title: Modify workflow function
description: Learn more about the modify workflow function within Knock's notification engine.
...
<Callout
  ...
  text={
    <>
      Modify workflow function is currently in beta. If you'd like early access,
      or this is blocking your adoption of Knock, please{" "}
      <a href="mailto:support@knock.app?subject=Modify workflow function">
        get in touch
      </a>
      .
    </>
...
When a modify workflow function executes, Knock will merge the configured properties into the `data` you originally passed to [the workflow trigger call](/send-notifications/triggering-workflows). Knock uses a shallow-merge strategy where:
...
_The merged data result from a modify workflow function step then becomes the global trigger data for all subsequent steps in the workflow run._

The example below illustrates how this could look in practice.

```json title="Example data merge for modify workflow function steps"
...
// Modify workflow function configuration
...
You can use the [workflow run logs](/send-notifications/debugging-workflows) to debug your modify workflow function steps. For each modify workflow function, you can expect to see in the logs:
...
<AccordionGroup>
  <Accordion title="When should I use a modify workflow function vs. a fetch function?">
  ...
  <Accordion title="Can I reference data from a previous modify workflow step?">
