...
<Callout
  type="warning"
  title="Message delivery is asynchronous from workflow execution."
  text={
    <>
      This means that message status conditions are most effective when used in
      combination with workflow steps (like a{" "}
      <a href="/designing-workflows/delay-function">delay function</a>) that
      pause workflow execution. See the section on{" "}
      <a href="#evaluation-timing">evaluation timing</a> below for more
      information.
    </>
  }
/>
