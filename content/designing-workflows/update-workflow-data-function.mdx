...
description: Learn more about the update workflow data function within Knock's notification engine.
...
<Callout
  ...
  text={
    <>
      Update workflow data function is currently in beta. If you'd like early
      access, or this is blocking your adoption of Knock, please{" "}
      <a href="mailto:support@knock.app?subject=Update workflow data function">
        get in touch
      </a>
      .
    </>
...
When an update workflow data function executes, Knock will merge the configured properties into the `data` you originally passed to [the workflow trigger call](/send-notifications/triggering-workflows). Knock uses a shallow-merge strategy where:
...
_The merged data result from an update workflow data function step then becomes the global trigger data for all subsequent steps in the workflow run._

The example below illustrates how this could look in practice.

```json title="Example data merge for update workflow data function steps"
...
// Update workflow data function configuration
...
You can use the [workflow run logs](/send-notifications/debugging-workflows) to debug your update workflow data function steps. For each update workflow data function, you can expect to see in the logs:
...
<AccordionGroup>
  <Accordion title="When should I use an update workflow data function vs. a fetch function?">
    Use an **update workflow data function** when you need to transform, compute, or restructure data that is already available in your workflow run. This is ideal for operations like combining fields, setting defaults, or pre-computing Liquid expressions.
    ...
    In some cases, you might use both: a fetch function to retrieve additional data, followed by an update workflow data function to transform or restructure the combined data for your templates.
  ...
  <Accordion title="What happens if a Liquid expression fails?">
    If a Liquid expression in your update workflow data function encounters an error (such as referencing a property that doesn't exist), the step will fail and halt your workflow run. You can use Liquid's default filter (e.g., `{{ data.optional_field | default: "fallback" }}`) to provide fallback values and avoid errors when data may be missing.
  </Accordion>
  <Accordion title="Can I reference data from a previous update workflow data step?">
    Yes. Since each update workflow data function merges its computed data into the workflow's global data state, subsequent steps (including other update workflow data functions) can reference that data using the `data` namespace. For example, if a previous step set `data.full_name`, a later step can reference it as `{{ data.full_name }}`.
  </Accordion>
</AccordionGroup>
