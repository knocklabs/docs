...
You can update a tenant's default [preferences](/preferences/overview) from within a workflow. Use the `preference_set` key (not `preferences`) when setting tenant preference defaults. The value should follow the structure of a [`PreferenceSet`](/api-reference/recipients/preferences/schemas/preference_set). For more information on how tenant preference defaults work and how to structure them, see [tenant preferences](/preferences/tenant-preferences).
...
In the step editor you can edit properties as a JSON blob or in key-value form. Updated properties are stored in Knock and are available to subsequent workflow steps and to the Management API. The updated tenant is immediately available to the next step and all later steps in the same workflow run.
