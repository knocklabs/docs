...
- **Recipient path.** The path on the incoming event that contains the Knock user ID. For [Segment](/integrations/sources/segment) events, use `userId`. For a [custom source](/integrations/sources/custom), use a path under `body.` or `headers.`, usually `body.userId`.

Your Knock environment must have at least one configured source that has received the event you want to wait for.

Knock always scopes matching to the workflow recipient for that run. Knock reads the user ID from the recipient path and resumes only waits registered for that recipient. If the path is blank or omitted, or the event has no value there, Knock does not resume any paused wait for that event.
...
Available message events include: created, queued, sent, not sent, delivered, delivery attempted, undelivered, bounced, complaint, read, unread, seen, unseen, archived, unarchived, interacted, and link clicked.
...
<AccordionGroup>
  <Accordion
    title="What happens if multiple events arrive while the workflow is waiting?"
    anchorSlug="faq-multiple-events-while-waiting"
  >
  ...
  <Accordion
    title="What happens if an event arrives after the wait time expires?"
    anchorSlug="faq-event-after-wait-expires"
  >
  ...
  <Accordion
    title="What happens if a source event has no recipient?"
    anchorSlug="faq-source-event-no-recipient"
  >
    Knock does not match the event to paused wait for event steps. Integration
    source matching is always recipient-scoped. If the recipient path is blank
    or omitted, or the event has no value at that path, the wait does not
    resume.
  </Accordion>
  <Accordion
    title="Do I need a source configured to use wait for event?"
    anchorSlug="faq-source-required-for-wait-for-event"
  >
  ...
  <Accordion
    title="Why are match conditions required for message and workflow events?"
    anchorSlug="faq-match-conditions-message-workflow-events"
  >
  ...
  <Accordion
    title="Can I use wait for event in an API-triggered workflow?"
    anchorSlug="faq-wait-for-event-api-triggered-workflows"
  >
  ...
  <Accordion
    title="Can I use wait for event in broadcasts?"
    anchorSlug="faq-wait-for-event-in-broadcasts"
  >
    Yes. Wait for event functions are supported in broadcasts as well as
    workflows.
  </Accordion>
  <Accordion
    title="Can a workflow wait for its own message events?"
    anchorSlug="faq-wait-for-own-message-events"
  >
  ...
  <Accordion
    title="What happens to paused workflow runs when I update the underlying workflow?"
    anchorSlug="faq-paused-runs-workflow-update"
  >
  ...
  <Accordion
    title="How long can a workflow wait for an event?"
    anchorSlug="faq-max-wait-for-event-duration"
  >
    A wait for event step can wait for a maximum of 30 days.
  </Accordion>
</AccordionGroup>
