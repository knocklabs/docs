...
- **Recipient path.** A JSON path into the source event that contains the Knock user ID. The path must start with `body.` or `headers.`, for example `body.userId` or `body.customerId`. If you leave the field blank, Knock uses `body.userId`. In the Management API, this field is `recipient_path`.

Your Knock environment must have at least one configured source that has received the event you want to wait for.

Knock always scopes matching to the workflow recipient for that run. Knock reads the user ID from the recipient path on the incoming event and resumes only waits registered for that recipient. An event with no resolvable recipient at that path does not match paused waits for that event key.
...
<AccordionGroup>
  ...
  >
    ...
    source matching is always recipient-scoped. Set the recipient path to the
    JSON path where the event stores the Knock user ID, and confirm incoming
    events include a value at that path.
  </Accordion>
  <Accordion
