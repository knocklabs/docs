...
When any of the checks above (conditions, recipient data, preferences, send windows) cause a step not to execute, the step is **skipped**. A skipped step does not produce a message, but the workflow continues to execute subsequent steps.

### When a step fails

If an error occurs during step execution—such as a template rendering failure or invalid channel configuration—Knock will retry the step up to **3 times**. If all retry attempts fail, the **entire workflow run is halted** and no subsequent steps will execute.
...
<Callout
  ...
  text={
    <>
      When a channel step fails after exhausting all retry attempts, the workflow stops immediately. See the <a href="/send-notifications/debugging-workflows#understanding-workflow-execution-behavior">debugging workflows documentation</a> for more details on failure vs. skip behavior.
    </>
  }
/>
