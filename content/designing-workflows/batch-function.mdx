...
A batch function always runs per recipient. When you provide a batch key, batching behavior is further grouped by that key. The batch key resolves to a value in your `data` payload and determines how incoming notifications are grouped for batching. The batch function runs per recipient across all notifications received by the workflow.
