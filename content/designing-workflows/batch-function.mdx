...
**_Initial batch window: 1 minute_**
...
This "Max extension limit" option is displayed once you enable a sliding window by selecting "Extend window when new activities are received," and can be set as any duration unit.
...
**_Initial batch window: 12 hours_**\
**_Max extension limit: 24 hours_**
...
**_Initial batch window: 24 hours_**\
**_Max extension limit: 12 hours_**
...
<Callout
  ...
  text={
    <>
      ...
      opened, it will remain open until its full duration has elapsed. This is
      because closing a batch window upon workflow cancellation would remove the
      capability to cancel individual workflow runs within a given batch.
      <br />
      <br />
      Because of this behavior, it's important to remember that <span className="font-bold">
        ...
        batch window itself.
      </span> Any subsequent triggers to that workflow key will add activities to
      the open batch, and those activities will proceed when the batch window closes
      if their respective workflow runs are not also canceled. See the FAQs below
      for a workaround to close an open batch window.
    </>
  }
/>
