...
You can combine multiple values when constructing a batch key. For example, setting the batch key to `{{data.eventType}}{{actor.id}}` would key would batch separately per event type and actor.
