...
Optionally, you can define **request inputs** for the reusable request so each workflow instance can supply its own structured values while sharing the same HTTP configuration. See [Request inputs](#request-inputs) later on this page.
...
Reusable requests can declare an **input schema** and optional **input values** so Knock knows what shape of per-workflow data you expect, validates saved values, and exposes that data to Liquid when the HTTP request is built.

#### Input schema

On the reusable request, **input schema** is a [JSON Schema](https://json-schema.org/) object that describes the allowed input properties (types, required fields, defaults, and so on).

When a schema is present, the dashboard can render structured editors for **Request inputs** on linked workflow fetch steps.
...
If the reusable request defines an input schema, preview inputs are validated against that schema when you save.

#### Per-workflow values on a linked fetch step

After you apply a reusable request to a fetch step, the workflow holds a **linked** step. The only fetch-related fields you can change on that linked step are its identity, conditions, and the per-workflow **Request inputs**.

Request inputs may contain Liquid. Knock renders Liquid using the normal workflow context (trigger `data`, `recipient`, `actor`, `vars`, and so on), so you can pass values that depend on each workflow run.
...
#### Validation and publishing order

- When the reusable request's **published** version includes an input schema, linked-step request inputs are validated against that schema when the workflow is saved.
- Knock reads the schema from the **published** reusable request, not from uncommitted drafts. Commit the reusable request first when you change the schema, then commit workflows that reference it.
