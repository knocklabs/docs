...
Knock allows you to create and save fetch functions so that you and your team can reuse them as instances across multiple workflows. Updates to these fetch functions cascade down to their instances, making it possible to centrally manage common configurations. Fetch functions are [a versionable resource](/concepts/commits) in Knock.

### Creating a reusable fetch function

To create a reusable fetch function, navigate to the **Fetch requests** page in the Knock dashboard from the main sidebar. Then, click the "Create request" button. After creation, this will take you to the function editor where you can specify the details of your function.
...
<Steps>
  ...
  <Step title="Select the request template">
    Once within the request editor, you can select the "Apply template" button to inherit the details of that reusable request. Now, **the workflow's fetch step will be in an uneditable state** with the details of the reusable request applied.
...
If you need to make changes to the request, it's recommended to edit the underlying request itself via the "Fetch requests" page, that way changes will apply to all instances of the request. However, if you need to make changes to just this instance, you can click the "Detach instance" button from the dropdown to remove the reusable fetch function usage, and enable an editable state.
