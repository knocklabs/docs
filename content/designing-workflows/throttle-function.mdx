...
You can combine multiple variables when constructing a throttle key. For example, setting the throttle key to `{{data.eventType}}{{actor.id}}` would throttle separately per event type and actor.
