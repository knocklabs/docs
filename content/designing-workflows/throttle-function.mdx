...
A throttle function always runs per recipient. When you provide a throttle key, throttling behavior is further grouped by that key. The throttle key resolves to a value in your `data` payload, or you can use Liquid to reference `recipient`, `actor`, or `tenant` namespaces. The key determines how incoming notifications are grouped for throttling. The throttle function runs per recipient across all notifications received by the workflow.
