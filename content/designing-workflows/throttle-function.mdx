...
You can combine multiple values when constructing a throttle key. For example, setting the throttle key to `{{data.eventType}}{{actor.id}}` would key would throttle separately per event type and actor.
