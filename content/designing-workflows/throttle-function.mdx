...
A throttle function always throttle events per `recipient`. When you provide a throttle key, throttled events are further grouped by that key. The throttle key resolves to a value in your `data` payload. You can also use Liquid to [reference variables](/designing-workflows/template-editor/variables) from the workflow run scope (such as the `recipient.*`, `actor.*`, or `tenant.*` namespaces) to construct a key.
