...
A delay function does just what it sounds like: it delays the execution of the workflow for some amount of time, then proceeds to the next step. There are three types of delays we support in Knock today: "wait for fixed interval", "wait for a dynamic duration", and  "wait until a relative timestamp."
...
- A correlated duration unit (e.g `{ "unit": "seconds", "value": 30 }`)
...
You can use our relative delay to wait some time before or after a timestamp that you provide in your workflow payload. Relative delays are great for use cases such as reminders, where you want to wait until a day before a due date to remind your users.
