...
#### Window rule properties
    ...
    | `day_of_month` | number (optional)                    | The day of the month when a monthly rule should execute. If omitted, it defaults to the day the delay window opened, and the rule executes at the next occurrence of the configured time. |
