...
You can set a relative delay to compute a delay time from a set interval and a dynamic duration. This means you can set a delay that occurs a set amount of time before or after a provided dynamic timestamp. This allows the creation of flexible and context-aware delay periods in your workflows.

When configuring a relative delay, you'll specify:

- The set interval (e.g., 1 day, 2 hours, 30 minutes)
- Whether the delay should occur before or after the dynamic duration
- The `key` for the dynamic duration (which can come from your trigger data, recipient data, or other sources)

As in the dynamic duration section above, the key specified must be available to be resolved. If the key is missing or resolves to an invalid value, a corresponding error will be logged on the workflow run, and the delay will be **skipped**.

Relative interval delay functions are helpful for various scenarios, including:

- Appointment reminders: Send a notification 1 day before an appointment time
- Follow-up reminders: Send a follow-up message 2 hours after an event
