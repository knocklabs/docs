...
| `cancellation_key` | string | None; dashboard prefills inheritance | Unique identifier used to cancel nested workflow runs. |

### Inheriting the parent cancellation key

New trigger workflow steps added in the dashboard prefill the cancellation key with `{{ workflow.cancellation_key }}`. This expression passes the current parent run's cancellation key to the nested workflow. If the parent run has no cancellation key, the nested workflow receives no key.

You can replace the expression with a custom string or another Liquid expression. Clearing the field sends no cancellation key; a blank field does not inherit the parent's key. Existing steps with blank cancellation keys remain unchanged. To enable inheritance for an existing step, set its cancellation key to `{{ workflow.cancellation_key }}` and commit the workflow.

When creating steps through the Management API or CLI, set `settings.cancellation_key` to `{{ workflow.cancellation_key }}` to enable inheritance. Omitting the field from the new step's settings sends no key.

When the trigger workflow step follows a [batch step](/designing-workflows/batch-function), `{{ workflow.cancellation_key }}` is the cancellation key of the run that opened the batch.
...
Sharing a cancellation key does not cause cancellation to cascade. To cancel a nested workflow that has already started, make a separate cancellation request using the **nested workflow's key** and its cancellation key. Canceling the parent workflow after the trigger workflow step executes does not cancel the nested workflow.
