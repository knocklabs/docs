...
There are also a number of [Liquid filters](/designing-workflows/template-editor/api-reference-liquid-helpers) you can use to mutate the variables you pass into a notification template. Here's an example that uses the `split` and `first` filters to pull the first name for a given user.
...
To learn more about the variables, Liquid keywords, and other helper functions available to you in the Knock template editor, check out our [liquid helper reference](/designing-workflows/template-editor/api-reference-liquid-helpers).
