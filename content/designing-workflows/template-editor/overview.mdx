...
All variables sent in the `data` payload of your workflow trigger call. These don't require a prefixed namespace to reference, so if you send through `{ "a_variable": "something" }` in your data payload you can reference this as `{{ a_variable }}` in your template.

<Callout
  emoji="💡"
  text={
    <>
      Note: if you want to reference all of the data passed to the workflow, you can use <code>$data</code> variable.
    </>
  }
/>
