...
<Steps titleSize="h3">
  <Step title="Add a domain">
    ...
    <Callout
      type="warning"
      title="Use a unique subdomain for the preference center."
      text={
        <>
...
Assignment changes take effect immediately for both tracked link and preference center domains. New messages will use the currently-assigned custom domains for that environment. Already-sent messages are not affected.
...
<AccordionGroup>
  <Accordion title="I've added the CNAME record successfully. Why isn't my domain marked as verified?">
    ...
    also try to verify the domain again by clicking the "Check verification" button.
  ...
  <Accordion title="Will preference center links stop working if I unassign my custom domain?">
    ...
    intact. Links stop working only if you delete the domain in Knock or modify the DNS record.
  </Accordion>
  ...
  <Accordion title="Does a preference center custom domain affect commercial unsubscribe links?">
    No. [Commercial unsubscribe](/preferences/commercial-unsubscribe) works independently and is not
