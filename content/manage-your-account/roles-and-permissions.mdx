...
A group's rule for a parent environment also applies to that environment's branches. Denying access to an environment clears obfuscation for that environment.
...
If an environment has customer data obfuscation enabled, that setting applies to every member and cannot be overridden by a permission group. Built-in roles have no per-role obfuscation setting. Only the environment setting applies to them.
