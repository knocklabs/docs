...
<Callout
  ...
  text={
    <>
      If you delete an asset that is referenced in a notification template, the image will no longer be displayed. Make sure to update any templates that reference an asset before deleting it.
    </>
  }
/>
...
<Accordion title="What file types are supported?">
  Currently, only image files are supported. This includes common formats like PNG, JPEG, GIF, and SVG. In the future, we may support additional asset types.
</Accordion>

<Accordion title="Is there a limit to how many assets I can upload?">
  There is no limit to the number of assets you can upload to your Knock account.
</Accordion>
