...
<Callout
  ...
  text={
    <>
      a user cannot change their own role or remove themself from a Knock
      account
    </>
  }
/>
...
A member's role can be changed from the three dot menu and selecting the "Change role" option. You can pick from one of the [available account roles](/manage-your-account/roles-and-permissions) and the role change will immediately take effect.

<Callout
  type="info"
  title="Note:"
  style={{ alignItems: "center" }}
  text={<>the last owner on an account cannot have their role changed</>}
/>
...
<Callout
  ...
  title="Note:"
  style={{ alignItems: "center" }}
  text={<>the last owner on an account cannot be removed</>}
/>
