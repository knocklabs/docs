...
It's possible to pass filters to the in-app messages endpoint. This makes it possible to:

- Only return in-app messages for a particular tenant
- Only return archived or unarchived messages
- Filter messages by the `data` associated
...
Each in-app message will contain the contents of the underlying in-app message template that was used to generate the message, which you can use to display your in-app notifications which you'll find under the `blocks` attribute on the `FeedItem` returned. Each block includes:
