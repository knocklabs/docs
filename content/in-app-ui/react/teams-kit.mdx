...
description: How to let users authorize and select Microsoft Teams channels in your app with Knock's TeamsKit.
section: TeamsKit
---

Our `@knocklabs/react` library comes with pre-built components for allowing your users to connect their Microsoft Teams tenants to Knock. TeamsKit manages your OAuth connection and tokens, and integrates seamlessly with the rest of Knock.
...
- [Sending messages to public channels](/integrations/chat/microsoft-teams/sending-a-message-to-channels) in your customer's Microsoft Teams tenants
- [Sending direct messages](/integrations/chat/microsoft-teams/sending-a-direct-message) to users in your customer's Microsoft Teams tenants
...
Once you've provided access to the necessary data, you can drop Knock's pre-built components into your React application to immediately set up Microsoft Teams authorization for your users. [See the reference](/in-app-ui/react/reference##microsoft-teams-components) for full documentation of these components.
...
<figure>
  <Image
    ...
    height={564}
  />
  <figcaption>The MsTeamsAuthButton component with MsTeamsAuthContainer</figcaption>
</figure>

Your users will give your Microsoft Teams bot access to their own Microsoft Teams tenants via the `MsTeamsAuthButton`. This button can be used on its own, or nested in the `MsTeamsAuthContainer` for a bigger visual footprint. Here's an example of how to use them:
...
The `MsTeamsAuthButton` maps a tenant in your product to a customer's Microsoft Teams tenant. This means in most cases you'll just need a single instance of the `MsTeamsAuthButton`.
...
- `knock.msTeams.revokeAccessToken`: Removes the Microsoft Teams tenant ID from the tenant
...
- [Microsoft Teams revoke access](/reference#ms-teams-revoke-access): remove a Microsoft Teams tenant ID from a Knock tenant
...
You'll need to generate a token for your user that includes access to the tenant storing the Microsoft Teams tenant ID as well as any recipient objects storing Teams channel data as described in [Microsoft Teams notifications with Knock](/integrations/chat/microsoft-teams/overview#channel-data-requirements). If you need to enable access to multiple recipient objects, you can include multiple grants in the user token.
...
- **The tenant**: the user needs access to this because this is where Knock stores the Microsoft Teams tenant ID (`ms_teams_tenant_id`) that will be used to send a notification as your Microsoft Teams bot
