...
The feed-specific provider that connects to a feed for that user. Must be a child of the `KnockProvider`. The `FeedClientOptions` object can contain any properties accepted by the [user feed API endpoint](https://docs.knock.app/reference#query-parameters-5).
...
<Attributes>
  <Attribute
    name="slackChannelRecipientObject*"
...
<Attributes>
  ...
  <Attribute
    name="redirectUrl"
    type="string"
    description="The URL to redirect back to after authorizing; should be your own applcation's URL"
  />
</Attributes>
...
<Attributes>
  <Attribute
    ...
    type="ContainerObject"
    description="The object that holds connected Slack channesl as channel data"
  />
</Attributes>
