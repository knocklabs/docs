...
For maximum flexibility while still leveraging Knock's functionality, you can use the individual subcomponents to compose your own modal. The SDK exports subcomponents as members of the `ModalView` object. This approach gives you full control over the layout and styling while maintaining the guide behavior:

```tsx title="Compose your own modal with subcomponents"
import { useGuide, ModalView } from "@knocklabs/react";
...
const CustomModal = () => {
  ...
  const { title, body, primary_button, secondary_button } = step.content;

  return (
    <ModalView.Root onOpenChange={(open) => !open && step.markAsArchived()}>
      <ModalView.Overlay />
      <ModalView.Content style={{ background: "#eef2ff" }}>
        <ModalView.Close />
        <ModalView.Title title={title} style={{ fontSize: "18px" }} />
        <ModalView.Body body={body} />

        <ModalView.Actions>
          {secondary_button && (
            <ModalView.SecondaryButton
...
The available `ModalView` subcomponents are:

| Subcomponent                | Renders                                                                                                                                              |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ModalView.Default`         | The full default modal. Takes a `content` object, plus optional `colorMode`, `onOpenChange`, `onDismiss`, `onButtonClick`, and `onImageClick` props. |
| `ModalView.Root`            | The Radix dialog root and portal. Opens by default and reports closes through `onOpenChange`.                                                        |
| `ModalView.Overlay`         | The backdrop behind the modal.                                                                                                                       |
| `ModalView.Content`         | The modal container.                                                                                                                                 |
| `ModalView.Title`           | The text passed to its `title` prop.                                                                                                                 |
| `ModalView.Body`            | The HTML passed to its `body` prop.                                                                                                                  |
| `ModalView.Img`             | An image. Takes the standard `img` props, such as `src` and `alt`.                                                                                   |
| `ModalView.Actions`         | A wrapper for the buttons.                                                                                                                           |
| `ModalView.PrimaryButton`   | A primary button labeled with its `text` prop.                                                                                                       |
| `ModalView.SecondaryButton` | A secondary button labeled with its `text` prop.                                                                                                     |
| `ModalView.Close`           | A close button that dismisses the dialog.                                                                                                            |
