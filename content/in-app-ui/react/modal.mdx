...
const CustomModal = () => {
  ...
  return (
    <ModalView.Root step={step}>
      <ModalView.Overlay onClick={() => step.markAsArchived()} />
      <ModalView.Content
        style={{
      ...
      >
        ...
        />
        <ModalView.Title style={{ color: "white", fontSize: "20px" }} />
        <ModalView.Body style={{ color: "rgba(255, 255, 255, 0.9)" }} />

        <ModalView.Actions>
          {step.content.secondary_button && (
...
- `ModalView.Root` — The container component that provides context to child components. Requires the `step` prop.
- `ModalView.Overlay` — The backdrop overlay behind the modal.
- `ModalView.Content` — The main modal container.
- `ModalView.Title` — Renders the modal title from `step.content.title`.
- `ModalView.Body` — Renders the modal body from `step.content.body`.
- `ModalView.Img` — Renders an image if included in the modal content.
- `ModalView.Actions` — Container for action buttons.
- `ModalView.PrimaryButton` — Renders the primary action button from `step.content.primary_button`.
- `ModalView.SecondaryButton` — Renders the secondary action button from `step.content.secondary_button`.
- `ModalView.Close` — Renders a close button that archives the guide when clicked.
