...
By default, the feed is initialized in `compact` mode. If your application needs the full payload, pass `{ mode: "rich" }` as the third argument to `useNotifications`:

| Mode      | Behavior                                                                                                                                                   |
| --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `compact` | Default. Returns a leaner payload — omits `activities`, `total_activities`, and all but one actor. Nested arrays and objects are also omitted from `data`. |
| `rich`    | Includes `activities`, `total_activities`, all actors.                                                                                                     |
...
See [FeedItem](/in-app-ui/api-overview#feeditem) for details on what each mode returns.
