...
Using our `@knocklabs/react` library, you can create custom notification preference interfaces backed by Knock's preferences API. The `usePreferences` hook fetches and updates preference data, manages cached data, and revalidates it after updates.

This page covers the React-specific steps for authenticating a user and building a custom preference interface with `usePreferences`. For the underlying API flow and configuration options, see [Custom preference center](/preferences/custom-preference-center).
...
- A signed user token from your backend (required in production with enhanced security mode enabled; see [Authentication with enhanced security](/in-app-ui/security-and-authentication#authentication-with-enhanced-security))
...
First, we'll need to implement the `KnockProvider` component somewhere in your component tree and authenticate against the Knock API using a user id, public API key, and signed user token.
...
In development environments without enhanced security mode enabled, you can omit the `userToken` prop and pass only the user id.
    ...
    <div className="workflow-preferences">
      ...
      {Object.entries(preferences?.workflows || {}).map(
        ([workflowKey, setting]) => {
          ...
          // Handle workflow preferences with channel type and/or channel settings
          if (
            typeof setting === "object" &&
            (setting?.channel_types || setting?.channels)
          ) {
            return (
              <div key={workflowKey} className="workflow-with-channels">
                ...
                {setting.channel_types && (
                  <div className="channel-types-section">
                    <h4>Channel types</h4>
                ...
                {setting.channels && (
                  <div className="specific-channels-section">
                    <h4>Specific channels</h4>
                    {Object.entries(setting.channels).map(
                      ([channelId, channelEnabled]) => (
                        <div key={channelId} className="channel-preference">
                          <label>
                            <input
                              type="checkbox"
                              checked={channelEnabled}
...
const CategoryPreferences = ({
  ...
  // Helper function for cleaner channel type preference updates
  ...
  // Helper function for specific channel preference updates
    ...
    setPreferences({
      ...
      categories: {
        ...
        [categoryKey]: {
          ...preferences?.categories?.[categoryKey],
          channels: {
            ...preferences?.categories?.[categoryKey]?.channels,
            [channelId]: enabled,
          },
        },
      },
      ...
      {Object.entries(preferences?.categories || {}).map(
        ([categoryKey, category]) => (
          <div key={categoryKey} className="category-section">
            ...
            {/* Specific channel preferences */}
            {category.channels && (
              <div className="specific-channels">
                <h4>Specific channels</h4>
                    ...
                    <div key={channelId} className="channel-preference">
                      <label>
                        <input
                          ...
                          checked={enabled}
                          onChange={(e) =>
                            updateCategorySpecificChannelPreference(
                              categoryKey,
                              channelId,
                              e.target.checked,
                            )
...
const PreferenceCenter = () => {
  ...
  const handleSpecificChannelToggle = (channelId, enabled) => {
    setPreferences({
      ...preferences,
      channels: {
      ...
      <div className="preference-content">
        ...
        {activeTab === "channels" && (
          <div className="channel-preferences">
            ...
            <div className="specific-channels-section">
              <h3>Specific channels</h3>
              <p>
                Control individual channel instances using their UUID
                identifiers (takes precedence over channel types)
              </p>
              {Object.entries(preferences?.channels || {}).map(
                ([channelId, enabled]) => (
                  <div key={channelId} className="channel-toggle">
                    <label>
                      <input
                        type="checkbox"
                        checked={enabled}
                        onChange={(e) =>
                          handleSpecificChannelToggle(
                            channelId,
                            e.target.checked,
                          )
                        }
                      />
                      Enable channel {channelId}
                    </label>
                  </div>
                ),
              )}
...
- [Custom preference center](/preferences/custom-preference-center)
- [`usePreferences` reference](/in-app-ui/react/sdk/hooks/use-preferences)
