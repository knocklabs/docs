---
title: "Build your own preferences UI (headless)"
...
Using our `@knocklabs/react` library, you can create fully custom notification preference UIs that are backed by Knock's preferences API. The `usePreferences` hook enables you to fetch and update user preferences with real-time caching, making it easy to build responsive preference centers.

In this guide, we'll walk through creating a completely custom preferences UI in our application using the `usePreferences` hook.
...
<Callout
  ...
  text={
    <>
      Before building custom preference UIs, we recommend reading the{" "}
      <a href="/preferences/overview">preferences overview</a> to understand
      how Knock's preference system works, including preference sets, workflows,
      and channel types.
    </>
  }
/>
...
<Callout
  ...
  text={
    <>
      When updating preferences, you can use the spread operator to merge changes
      with existing preferences. This approach is cleaner than manually
...
Workflow preferences control whether users receive notifications for specific workflows. Here's how to build a UI for workflow preferences:
...
const WorkflowPreferences = () => {
  ...
  return (
    <div className="workflow-preferences">
      ...
      {Object.entries(preferences?.workflows || {}).map(([workflowKey, setting]) => {
            ...
            <div key={workflowKey} className="preference-item">
              <label>
                <input
                  type="checkbox"
                  checked={setting}
                  onChange={(e) => handleWorkflowToggle(workflowKey, e.target.checked)}
                />
...
Category preferences let users control notifications for groups of related workflows. Here's how to build a category preferences interface:
...
const CategoryPreferences = () => {
  ...
  const updateCategoryChannelPreference = (categoryKey, channelType, enabled) => {
    setPreferences({
      ...preferences,
      categories: {
              ...
              <div key={channelType} className="channel-preference">
                <label>
                  <input
                    ...
                    checked={enabled}
                    onChange={(e) => 
                      updateCategoryChannelPreference(categoryKey, channelType, e.target.checked)
                    }
                  />
                  {channelType} notifications
                </label>
...
const PreferenceCenter = () => {
  const { preferences, setPreferences, isLoading, isValidating } = usePreferences();
      ...
      <div className="preference-content">
        ...
        {activeTab === "channels" && (
          <div className="channel-preferences">
            ...
            <p>Control which types of channels you receive notifications through.</p>
              ...
              <div key={channelType} className="channel-toggle">
                <label>
                  <input
                    ...
                    checked={enabled}
                    onChange={(e) => 
                      handleBulkChannelTypeToggle(channelType, e.target.checked)
                    }
                  />
...
const AdvancedPreferences = () => {
  ...
  return (
    <div>
      <button onClick={handleExportPreferences}>
...
You now have the building blocks to create sophisticated preference UIs with Knock. Consider:
...
- Creating preset configurations that users can quickly apply
