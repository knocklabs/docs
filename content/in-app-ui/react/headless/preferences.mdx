...
This page is a React-specific implementation of [Build your preference center](/preferences/build-your-preference-center). The `usePreferences` hook wraps the same preferences APIs documented there—public API key authentication, signed user token, and `preferences.get()` / `preferences.set()` under the hood.
