...
For maximum flexibility while still leveraging Knock's functionality, you can use the individual subcomponents to compose your own banner. The SDK exports subcomponents as members of the `BannerView` object. This approach gives you full control over the layout and styling while maintaining the guide behavior:

```tsx title="Compose your own banner with subcomponents"
import { useGuide, BannerView } from "@knocklabs/react";
...
const CustomBanner = () => {
  ...
  return (
    <BannerView.Root style={{ background: "#eef2ff" }}>
      <BannerView.Content>
        <BannerView.Title title={title} style={{ fontWeight: 600 }} />
        <BannerView.Body body={body} />
      </BannerView.Content>

      <BannerView.Actions>
        {secondary_button && (
          <BannerView.SecondaryButton
        ...
        {primary_button && (
          <BannerView.PrimaryButton
            text={primary_button.text}
            onClick={() => step.markAsInteracted()}
          />
        )}

        <BannerView.DismissButton onClick={() => step.markAsArchived()} />
      </BannerView.Actions>
    </BannerView.Root>
  );
};
...
The available `BannerView` subcomponents are:

| Subcomponent                 | Renders                                                                                                               |
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `BannerView.Default`         | The full default banner. Takes a `content` object, plus optional `colorMode`, `onDismiss`, and `onButtonClick` props. |
| `BannerView.Root`            | The outer banner container.                                                                                           |
| `BannerView.Content`         | A wrapper for the title and body.                                                                                     |
| `BannerView.Title`           | The text passed to its `title` prop.                                                                                  |
| `BannerView.Body`            | The HTML passed to its `body` prop.                                                                                   |
| `BannerView.Actions`         | A wrapper for the buttons.                                                                                            |
| `BannerView.PrimaryButton`   | A primary button labeled with its `text` prop.                                                                        |
| `BannerView.SecondaryButton` | A secondary button labeled with its `text` prop.                                                                      |
| `BannerView.DismissButton`   | A dismiss button. Wire up `onClick` to archive the guide.                                                             |
