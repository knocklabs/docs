...
const CustomBanner = () => {
  ...
  return (
    ...
    >
      <BannerView.Content>
        <BannerView.Title style={{ fontWeight: "bold" }} />
        <BannerView.Body />
      </BannerView.Content>
...
- `BannerView.Root` — The container component that provides context to child components. Requires the `step` prop.
- `BannerView.Content` — Wraps the title and body content.
- `BannerView.Title` — Renders the banner title from `step.content.title`.
- `BannerView.Body` — Renders the banner body from `step.content.body`.
- `BannerView.Actions` — Container for action buttons.
- `BannerView.PrimaryButton` — Renders the primary action button from `step.content.primary_button`.
- `BannerView.SecondaryButton` — Renders the secondary action button from `step.content.secondary_button`.
- `BannerView.DismissButton` — Renders a dismiss button that archives the guide when clicked.
