...
This page is a React-specific walkthrough for [Build your preference center](/preferences/build-your-preference-center). It uses the same auth pattern (public API key + signed user token) and the same `user.getPreferences()` / `user.setPreferences()` APIs to build a `PreferenceCenter` component. This tutorial covers the default user preference set; see the build page for [per-tenant](/multi-tenancy/per-tenant-preferences) and [object](/preferences/object-preferences) preferences. For a TypeScript reference, see the <a href="https://github.com/knocklabs/notion-feed-example/blob/main/components/PreferenceCenter.tsx" target="_blank" rel="noopener">Notion feed example</a>.
...
Before you get started, we recommend reading the [preferences overview docs](/preferences/overview) and creating a [default `PreferenceSet`](/preferences/build-your-preference-center#create-a-default-preferenceset) for your environment. The [API reference for preferences](/api-reference/recipients/preferences) can also be helpful.
