...
First, we need to load the current user's preferences from Knock and store them in local state so we can operate on them. We can call the `get` method on `knockClient.preferences` to load a user's preferences:
...
Based on the `preferenceType` of the update, we'll overwrite any existing preferences stored under that key and use the `set` method on `knockClient.preferences` to update those preferences in Knock. We then set the updated preferences back to local state using `setLocalPreferences` to keep our UI in sync with what is stored in Knock.
