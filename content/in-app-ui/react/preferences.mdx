...
In this guide, we will build a `PreferenceCenter` React component with Knock's preference APIs. This component is flexible enough to handle most needs and can be customized or extended for specific use cases. For a TypeScript implementation, see the [Notion feed example](https://github.com/knocklabs/notion-feed-example/blob/main/components/PreferenceCenter.tsx).
...
Before beginning this tutorial, read the [preferences overview docs](/preferences/overview) and create a [default `PreferenceSet`](/preferences/overview#create-a-default-preferenceset) for your environment. The [API reference for preferences](/reference#preferences) provides additional context but is not required for this tutorial.
...
<Callout
  ...
  text={
    <>
      <span className="font-bold">Merging preferences.</span>
      <p>
        If you have either an environment or tenant default{" "}
        <code>PreferenceSet</code>, those preferences will be merged with
        changes a user makes in the UI—with user-specified changes taking
        precedence.
      </p>
    </>
  }
...
In this example, we assume the user has a default `PreferenceSet` containing workflows and workflow categories, each with its own channel type settings. We expose this to users as a "cross-hatch" so they can set a preference for each channel type.
...
const onPreferenceChange = async ({
  ...
  // Update preference set based on type and override channel settings—
  // since Workflow and Category preferences can be Boolean,
  // check if preferenceKey contains a channel_types object
  if (
    preferenceType === "category" &&
    typeof preferenceUpdate.categories[preferenceKey] === "object"
  ) {
...
// PreferenceSettingsRow component—displays UI for preference manipulation
function PreferenceSettingsRow({
  preferenceType,
  preferenceKey,
  channelTypeSettings,
