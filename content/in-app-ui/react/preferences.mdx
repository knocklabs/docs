...
The next step here is to create our preference center component. Create a `PreferenceCenter.jsx` file in your project and add the following import statements to the top of the file.
    ...
    return (
      <div>
        ...
        {Object.keys(PreferenceViewConfig.ChannelTypeLabels).map(
          (channelType) => {
            return (
              <div key={`${preferenceKey}_${channelType}`}>
...
```
  ...
  <Step title="Render your preference center">
    Now that we have a `PreferencesViewConfig` object to help us drive the shape of our UI and a `PreferenceSettingsRow` to render a row's details an the necessary `inputs`, it's time to compose those elements into an actual `PreferenceCenter` component. This `PreferenceCenter` should be exported:
...
Let's step through the code in the `PreferenceCenter` component step-by-step to explain what's happening.
...
Next, we create a function called `onPreferenceChange` that will get passed as the `onChange` paramter to our `PreferenceSettingsRow` component from the previous step. The `onPreferenceChange` function takes a `preferenceKey` argument and an updated `channelTypeSettings` argument:
...
Knock's preference model is very flexibile, but you should find that the component below will satisfy most of your preference center needs and can easily be used as a starting point for your own preference center.
