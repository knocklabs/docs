...
const CustomCard = () => {
  ...
  return (
    ...
    >
      <CardView.Content>
        <CardView.Title style={{ color: "white", fontSize: "20px" }} />
        <CardView.DismissButton style={{ color: "rgba(255, 255, 255, 0.8)" }} />
      </CardView.Content>

      <CardView.Body style={{ color: "rgba(255, 255, 255, 0.9)" }} />
...
- `CardView.Root` — The container component that provides context to child components. Requires the `step` prop.
- `CardView.Content` — Wraps the title and other header content.
- `CardView.Title` — Renders the card title from `step.content.title`.
- `CardView.Body` — Renders the card body from `step.content.body`.
- `CardView.Img` — Renders an image if included in the card content.
- `CardView.Actions` — Container for action buttons.
- `CardView.PrimaryButton` — Renders the primary action button from `step.content.primary_button`.
- `CardView.SecondaryButton` — Renders the secondary action button from `step.content.secondary_button`.
- `CardView.DismissButton` — Renders a dismiss button that archives the guide when clicked.
