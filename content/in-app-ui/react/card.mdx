...
For maximum flexibility while still leveraging Knock's functionality, you can use the individual subcomponents to compose your own card. The SDK exports subcomponents as members of the `CardView` object. This approach gives you full control over the layout and styling while maintaining the guide behavior:

```tsx title="Compose your own card with subcomponents"
import { useGuide, CardView } from "@knocklabs/react";
...
const CustomCard = () => {
  ...
  return (
    <CardView.Root style={{ background: "#eef2ff" }}>
      <CardView.Content>
        <CardView.Title title={title} style={{ fontSize: "18px" }} />
        <CardView.DismissButton onClick={() => step.markAsArchived()} />
        <CardView.Body body={body} />
      </CardView.Content>

      <CardView.Actions>
        {primary_button && (
          <CardView.PrimaryButton
...
The available `CardView` subcomponents are:

| Subcomponent               | Renders                                                                                                                             |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `CardView.Default`         | The full default card. Takes a `content` object, plus optional `colorMode`, `onDismiss`, `onButtonClick`, and `onImageClick` props. |
| `CardView.Root`            | The outer card container.                                                                                                           |
| `CardView.Content`         | A wrapper for the title and body.                                                                                                   |
| `CardView.Title`           | The text passed to its `title` prop.                                                                                                |
| `CardView.Body`            | The HTML passed to its `body` prop.                                                                                                 |
| `CardView.Img`             | An image. Takes the standard `img` props, such as `src` and `alt`.                                                                  |
| `CardView.Actions`         | A wrapper for the buttons.                                                                                                          |
| `CardView.PrimaryButton`   | A primary button labeled with its `text` prop.                                                                                      |
| `CardView.SecondaryButton` | A secondary button labeled with its `text` prop.                                                                                    |
| `CardView.DismissButton`   | A dismiss button. Wire up `onClick` to archive the guide.                                                                           |
