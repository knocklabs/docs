...
Optionally a feed can be initialized with a default set of `FeedClientOptions` which will be applied to all subsequent requests. The `FeedClientOptions` object can contain any properties accepted by the [user feed API endpoint](https://docs.knock.app/reference#query-parameters-5).
