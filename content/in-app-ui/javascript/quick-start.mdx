...
The `Knock` class exposes a `Feed` via the `initialize` method that can be used to connect the authenticated user to a Knock Feed Channel.

The `initialize` method optionally takes a `FeedClientOptions` object that can contain any properties accepted by the [user feed API endpoint](https://docs.knock.app/reference#query-parameters-5).

Additionally, the `Feed` exposes a stateful store to build client-side feeds and other notification experiences.
...
knockFeed.fetch();
// Get the FeedStateStore
const store = knockFeed.getState();
```
