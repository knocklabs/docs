---
title: "Knock Android SDK"
description: Learn more about integrating Knock into your Android applications through our Android SDK.
section: SDKs
---

The Knock Kotlin SDK is a client-side SDK for interacting with the Knock API and for building notification experiences within Android applications.

**Quick links**

- [SDK on GitHub](https://github.com/knocklabs/knock-android)
- [Full reference documentation](/in-app-ui/android/sdk/reference)

## Example app

You can find a complete Android example application that uses the Knock Android SDK <a href="https://github.com/knocklabs/knock-android" target="_blank" rel="noopener">here</a>. The app shows patterns for handling push token registration, building an in-app feed using Combine, and managing user notification preferences.

## Need help?

Our Android SDK is worked on full-time by the Knock Mobile team.

### Join the community

- [Knock community Slack](https://knock.app/join-slack)

### Provide feedback

- [Open an issue](https://github.com/knocklabs/knock-android/issues/new)
- Click the "Contact support" button at the top of this page to reach our support team.

### Contributing

All contributors are welcome, from casual to regular. Feel free to open a <a href="https://github.com/knocklabs/knock-android/pulls/new" target="_blank" rel="noopener">pull request</a>.
