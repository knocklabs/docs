...
[**React**](/in-app-ui/react/sdk/overview)

- Pre-built UI components for building your own custom UI components
- A set of hooks for building your own custom UI components
- State management for guides

[**React Native**](/in-app-ui/react-native/sdk/overview)

- A set of hooks for building your own custom UI components
- State management for guides

[**Javascript**](/in-app-ui/javascript/sdk/overview)

- Low-level API bindings and state management for building custom UI components.

### Key resources for guides

When working with Knock's client-side SDKs to fetch and render guides, you'll use the following components and hooks:
...
## Fetching guides via API

You can also directly call the [guides API](/api-reference/users/guides) to fetch guides and render them in your product. This approach gives flexibility and full control over the rendering process, but requires you to build your own state management and UI components. We highly recommend using the SDKs as they provide a robust set of tools and helpers for working with guides.

## Filtering guides

You can fetch guides either by a `type` (corresponding to a message type _key_) or a `key` (a specific _guide_ _key_) to select a guide from the fetched set of guides. Here's how to think about when to use both of these:
...
## Rendering guide components

Once you've fetched guides, you can render them in your product using either custom components or Knock's pre-built components.

- **Custom components.** [Build your own guide components](/in-app-ui/react/headless/guide) to create a fully custom experience that matches your design system.
- **Pre-built components.** Use Knock's out of the box components like [Banner](/in-app-ui/react/banner), [Modal](/in-app-ui/react/modal), and [Card](/in-app-ui/react/card) to quickly render guides with minimal setup.

## Frequently asked questions

<AccordionGroup>
  <Accordion title="When should I use useGuide vs useGuides?">
    Use [`useGuide`](/in-app-ui/react/sdk/hooks/use-guide) when you want to fetch and render a single guide at a time. This is ideal for components that should only display one guide, like a banner or modal that occupies a specific location in your UI.

    Use [`useGuides`](/in-app-ui/react/sdk/hooks/use-guides) when you want to fetch and render multiple guides simultaneously. This is useful for components that can display multiple guides at once, like a list of changelog cards or announcements in a sidebar.
  ...
  <Accordion title="How do I render multiple guides in a single component?">
    To render multiple guides in a single component, use the [`useGuides`](/in-app-ui/react/sdk/hooks/use-guides) hook instead of [`useGuide`](/in-app-ui/react/sdk/hooks/use-guide). The plural `useGuides` hook returns an array of guides that you can map over to render multiple instances.
