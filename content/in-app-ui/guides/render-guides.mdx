...
Starting in version 0.10.0 of `@knocklabs/react`, `useGuides` respects [throttling rules](/in-app-ui/guides/order-guides#guide-throttling) by default, similar to `useGuide`. If you need to return all eligible guides regardless of throttling, you can pass `includeThrottled: true` as an option:

```tsx
const { guides } = useGuides({ type: "changelog" }, { includeThrottled: true });
```
