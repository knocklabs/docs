...
You can fetch guides either by a `type` or a `key` to select a guide from the fetched set of guides. Here's how to think about when to use both of these:

| Selector | Filters by         | When to use                                                                          |
| -------- | ------------------ | ------------------------------------------------------------------------------------ |
| `key`    | Guide `key`        | Render **a specific instance of a guide**.                                           |
| `type`   | Message type `key` | Render **any guide** that uses this [message type](/in-app-ui/guides/message-types). |

By default, when multiple guides exist for a single `type` (e.g, a generic "Banner" component that could be used multiple times across many guides) Knock returns the earliest created guides first then the most recently created guide last. Alternatively, you can control this behavior and manually configure the relative priority of guides by [ordering guides](/in-app-ui/guides/order-guides) in the Knock dashboard.
