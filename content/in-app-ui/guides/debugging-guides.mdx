...
<Callout
  ...
  text="Ensure your message type's template preview code matches your application code to avoid unexpected visual differences when the guide renders in your actual application. We recommend using live preview to ensure your actual code renders your guide schema as expected."
/>

### Testing best practices

- **Test page targeting**. Verify that guides appear on the correct pages by testing different URLs and path patterns in your activation rules.
- **Test user targeting**. Confirm that targeting conditions work correctly by testing with users who should and shouldn't see the guide based on your eligibility criteria.
- **Test engagement events**. Interact with your guides and verify that engagement status updates appear as expected in the dashboard (seen, interacted, archived).
- **Test all interactions**. Click all links, buttons, and interactive elements to ensure they work as expected and navigate to the correct destinations.
- **Use message unarchiving**. Take advantage of the "unarchive" option in the dashboard to reset guide messages, allowing you to test the same user/guide combination multiple times.
...
The expected results of engagment status updates will may vary. While Knock's pre-built guide components provide out-of-the-box engagement tracking, your team will own the UI of other guide components completely. This includes decisions about and the implementation of engagement status updates.

**Common engagement tracking issues:**

- Engagement status updates have not been implemented for the component.
- Engagement status requests fail because required parameters are missing or incorrect (see [guides API](/api-reference/users/guides/mark_message_as_seen)):
...
- Engagement status requests fail because the `guide_id` is used in the endpoint instead of `message_id` (see [guides API](/api-reference/users/guides/mark_message_as_seen)).
