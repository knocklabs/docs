...
Guides can receive runtime data from your application via the [guides API](/api-reference/guides/). You can use this data to personalize the guide's content and set its targeting rules.
...
TODO we should add a code example of passing data to a KnockGuideProvider here.
...
When creating your template content, you can make personalizations using [Liquid](https://docs.knock.app/designing-workflows/template-editor/overview) and any [data passed to the guides API](/guides#overview) at runtime, including `data`, `recipient`, `tenant`, and `vars` properties.
...
If you do not provide any activation rules, the guide will activate when you call the `useGuide` hook in your application. More on this below.
