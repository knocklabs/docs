...
| A Liquid value is empty or malformed                                           | Knock cannot render a valid parent message ID or destination, so no reply is sent.                      | Verify the workflow trigger data and Liquid path.                             |
