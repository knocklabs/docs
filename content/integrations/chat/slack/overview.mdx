...
<Steps titleSize="h3">
  <Step title="Set up Slack app">
    ...
    <AccordionGroup>
      <Accordion title="Add a scope" anchorSlug="add-a-scope">
      ...
      <Accordion title="Add redirect URL" anchorSlug="add-redirect-url">
      ...
      <Accordion title="Set up public distribution" anchorSlug="set-up-public-distribution">
...
If one recipient stores destinations for several tenants, add `knock_tenant_id` to each `SlackConnection`. When you trigger the workflow with a tenant, Knock uses connections tagged with that Knock tenant ID. If none match, Knock uses only untagged connections. This selection happens before Knock applies any access token stored on the tenant. See [tenant-scoped chat connections](/managing-recipients/setting-channel-data#scope-chat-connections-by-tenant) for the full selection behavior and an example.
...
<AccordionGroup>
  <Accordion title="SlackConnection with an access token" anchorSlug="slackconnection-with-an-access-token" description="For self-serve customer integrations">
  ...
  <Accordion title="SlackConnection with an incoming webhook url" anchorSlug="slackconnection-with-an-incoming-webhook-url" description="For internal or one-off integrations">
