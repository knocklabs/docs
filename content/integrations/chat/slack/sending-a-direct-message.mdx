...
That's it! You should now be able to send direct messages to your users in Slack using Knock.

## Frequently asked questions

<AccordionGroup>
  <Accordion title="How can I fix `The user token supplied is invalid` error when connecting to Slack?">
    This error typically indicates a 400 error where the user does not have permissions to write to Slack-specific resources in Knock like a tenant or object. Check that you are providing a valid user token to the `KnockProvider` component that carries grants for writing to a specific tenant or object.
