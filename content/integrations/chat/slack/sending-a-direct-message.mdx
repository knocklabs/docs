...
In this implementation, we'll actually store [the required channel data](/integrations/chat/slack/overview#recipient-channel-data-requirements) for a `SlackConnection` across two different entities in Knock: a `Tenant` and a `User`. This is because we want to store the `access_token` for the Slack workspace on the `Tenant` and the `user_id` for the Slack user on the `User`.
...
If a Knock user can have a different Slack user ID in each tenant, store one connection for each tenant and add the corresponding `knock_tenant_id` to it. Knock selects the matching connection before applying the Slack access token stored on the tenant. See [tenant-scoped chat connections](/managing-recipients/setting-channel-data#scope-chat-connections-by-tenant) for the selection rules and how to retain existing connections when setting channel data.
...
<AccordionGroup>
  <Accordion title="Enabling email-based user ID resolution on a Slack channel" anchorSlug="email-based-user-id-resolution-slack-channel">
    <Steps>
  ...
  <Accordion title="Enabling email-based user ID resolution on a step in a workflow" anchorSlug="email-based-user-id-resolution-workflow-step">
    <Steps>
