...
<Callout
  ...
  text={
    <>
      ...
      complete the OAuth flow again to re-authorize your app. Otherwise, the
      Slack API request will fail.
    </>
  }
/>
...
<AccordionGroup>
  <Accordion title="Enabling email-based user ID resolution on a Slack channel">
    In the dashboard, navigate to the **Integrations** tab and select your Slack
    integration. Click the **Manage configuration** button for one of your
    environments to open the environment configuration dialog. Locate the toggle
    labeled **Enable email-based user ID resolution** and toggle it on. Click
    the **Update settings** button to save your changes.
  </Accordion>
  <Accordion title="Enabling email-based user ID resolution on a step in a workflow">
    ...
    workflow. Click on an existing Slack step in the workflow. Under **Channel
    settings** in the right panel, locate the toggle labeled **Enable
    email-based user ID resolution** and toggle it on.
  </Accordion>
</AccordionGroup>
