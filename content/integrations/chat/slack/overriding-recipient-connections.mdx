...
Because the override replaces stored recipient connections, Knock does not apply [`knock_tenant_id` connection selection](/managing-recipients/setting-channel-data#scope-chat-connections-by-tenant) to an override. A tenant included in the workflow trigger can still provide the Slack access token when the override omits `access_token`.
