...
<Steps>
  ...
  <Step title="Select your bot’s type">
    ...
    <AccordionGroup>
      <Accordion title="Single-tenant bot" anchorSlug="single-tenant-bot" defaultOpen={true} >
      ...
      <Accordion title="Multi-tenant bot" anchorSlug="multi-tenant-bot">
...
If one recipient stores destinations for several tenants, add `knock_tenant_id` to each `MsTeamsConnection`. This field is a Knock tenant ID, not a Microsoft Entra tenant ID. When you trigger the workflow with a tenant, Knock uses connections tagged with that Knock tenant ID. If none match, Knock uses only untagged connections. This selection happens before Knock applies any `ms_teams_tenant_id` stored on the tenant. See [tenant-scoped chat connections](/managing-recipients/setting-channel-data#scope-chat-connections-by-tenant) for the full selection behavior and an example.
...
<AccordionGroup>
  <Accordion title="MsTeamsConnection as a bot" anchorSlug="msteamsconnection-as-a-bot" description="For self-serve customer integrations">
    ...
    | Property            | Type     | Description                                                                                                                                    |
    | ------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
    | ms_teams_tenant_id  | `string` | A Microsoft Entra tenant ID. Not required when the ID is [stored on a tenant](#tenant-channel-data-requirements).                                |
    ...
    | knock_tenant_id     | `string` | The optional Knock tenant ID that scopes this connection. This field is distinct from `ms_teams_tenant_id`.                                     |

  </Accordion>
  <Accordion title="MsTeamsConnection with an incoming webhook url" anchorSlug="msteamsconnection-with-an-incoming-webhook-url" description="For internal or one-off integrations">
