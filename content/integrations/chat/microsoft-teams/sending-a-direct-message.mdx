...
In this implementation, we'll actually store [the required channel data](/integrations/chat/microsoft-teams/overview#how-to-set-channel-data-for-a-microsoft-teams-integration-in-knock) for an `MsTeamsConnection` across two different entities in Knock: a `Tenant` and a `User`. This is because we want to store the `ms_teams_tenant_id` for the Microsoft Entra tenant on the Knock `Tenant` and the `ms_teams_user_id` for the Microsoft Teams user on the Knock `User`.
...
If a Knock user can have a different Microsoft Teams user ID in each tenant, store one connection for each tenant and add the corresponding `knock_tenant_id` to it. Knock selects the matching connection before applying the `ms_teams_tenant_id` stored on the tenant. See [tenant-scoped chat connections](/managing-recipients/setting-channel-data#scope-chat-connections-by-tenant) for the selection rules and how to retain existing connections when setting channel data.
      ...
      if (activity.conversation.conversationType === "personal") {
        ...
        knockClient.users
          .setChannelData(knockUserId, process.env.KNOCK_MS_TEAMS_CHANNEL_ID, {
            connections: [
              {
                ms_teams_user_id: msTeamsUserId,
