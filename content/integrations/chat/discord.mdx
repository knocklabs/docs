...
The main difference with both approaches is that for incoming webhooks, you need to create one per Discord channel. If you are planning to notify just a few Discord channels, this is probably the easiest, but if you need to notify multiple Discord channels the bot approach will work best. Additionally, <a href="https://discord.com/developers/docs/interactions/message-components" target="_blank" rel="noopener">Message Components</a> cannot be used with the incoming webhook approach, so if you're planning on leveraging those you should create a bot.
...
If we check the **Objects** page under the **Recipients** section in our Knock dashboard, we can see the project exists there. Now we can give it specific channel information so it knows where to post in Discord.
...
You'll need to set up a bot and app to handle incoming messages from Knock. You can learn more in the documentation for <a href="https://discord.com/developers/applications" target="_blank" rel="noopener">creating Discord apps</a> and <a href="https://discord.com/developers/docs/topics/oauth2#bots" target="_blank" rel="noopener">creating Discord bots</a> (note: you'll need to be logged in to access the applications docs).
    ...
    <>
      ...
      <ul>
        <li>
          A{" "}
          <a
            href="https://discord.com/developers/docs/game-sdk/applications#data-models"
            target="_blank"
            rel="noopener"
          >
            scope
        ...
        <li>
          <a
            href="https://discord.com/developers/docs/topics/oauth2#bot-authorization-flow"
            target="_blank"
            rel="noopener"
          >
            Bot permissions
...
## Route connections by tenant

If the same user or object needs a different Discord destination for each tenant in your application, add the optional `knock_tenant_id` field to each `DiscordConnection`. Set it to the corresponding [Knock tenant ID](/multi-tenancy/overview), then include that tenant when you trigger the workflow.

When the trigger includes a tenant, Knock uses matching tenant-scoped connections. If none match, Knock uses untagged connections as a fallback. You can scope bot-based connections and incoming webhooks. See [tenant-scoped chat connections](/managing-recipients/setting-channel-data#scope-chat-connections-by-tenant) for an example and the full selection behavior.
...
<Attributes>
  ...
  />
  <Attribute
    name="knock_tenant_id"
    type="string"
    description="The optional Knock tenant ID that scopes this connection."
  />
</Attributes>
