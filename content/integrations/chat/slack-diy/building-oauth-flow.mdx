...
<AccordionGroup>
  <Accordion title="Users" description="For notifying users via direct message">
    If your integration involves a user opting in to receive DMs from your Slack bot about information pertaining to them, you’ll be storing the channel data [on that user](/reference#set-user-channel-data) in Knock. Then when you want to notify this user, you'll include the user as a recipient in a Knock workflow trigger.

    For this integration, you'll store your user's Slack `user_id` in the `SlackConnection` object rather than a Slack `channel_id`. You can find the correct `user_id` by querying Slack's API: 
