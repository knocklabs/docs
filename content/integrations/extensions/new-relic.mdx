...
At this time there is no way to selectively enable specific metrics, however metrics will only be emitted to New Relic for features that you are actively using in Knock.
...
<Callout
  ...
  text={
    <>
     When creting an API key, make sure "Key Type" is marked as <strong> Ingest - License </strong>
    </>
  }
/>
