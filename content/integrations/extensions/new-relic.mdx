...
### Include tenant as a metric tag

To add a `tenant:<tenant_id>` tag to workflow and message metrics, enable **Include tenant as a metric tag** in the <a href="https://dashboard.knock.app/~/settings/integrations/extensions?extension-new-relic=true" target="_blank" rel="noopener">New Relic extension settings</a>. Knock includes this tag when a [tenant](/concepts/tenants) is associated with the workflow run or message. Integration source metrics do not include a tenant tag.
...
<Callout
  ...
  text="Tenant IDs can produce a large number of unique tag values. Enabling this setting increases metric cardinality and may increase your New Relic costs. Keep this setting off unless you need to filter or group Knock metrics by tenant."
/>
...
3. Enter a New Relic API Key from New Relic's <a href="https://one.newrelic.com/launcher/api-keys-ui.api-keys-launcher" target="_blank" rel="noopener">API Keys</a> page (we recommend creating a dedicated key just for Knock)
4. Pick the correct site for your New Relic data hosting (visit <a href="https://docs.newrelic.com/docs/accounts/accounts-billing/account-setup/choose-your-data-center/" target="_blank" rel="noopener">New Relic's docs</a> for more information)
5. Optionally enable **Include tenant as a metric tag** to add `tenant:<tenant_id>` tags to workflow and message metrics
6. Click "Connect"
...
1. Visit New Relic's <a href="https://one.newrelic.com/all-capabilities" target="_blank" rel="noopener">all capabilities page</a> and click on "Dashboards"
...
- When **Include tenant as a metric tag** is enabled, Knock also segments applicable workflow and message metrics by `tenant`
...
3. If you created a dedicated New Relic API key for Knock, you can now delete the key from New Relics's <a href="https://one.newrelic.com/launcher/api-keys-ui.api-keys-launcher" target="_blank" rel="noopener">API Keys</a> page
