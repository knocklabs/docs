...
3. Click the button below to copy the dashboard JSON. Before pasting, replace all instances of `0` in the JSON with your New Relic account ID.
   - Your New Relic account ID can be found by either locating it in the URL after `/accounts/` or by opening the user menu in the bottom-left corner and go to **Administration** > **Access management** > **Accounts** to view IDs for all accounts you have access to.
4. Paste the updated JSON into the New Relic page when prompted.
