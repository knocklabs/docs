...
To add a `tenant:<tenant_id>` tag to workflow and message metrics, enable **Include tenant as a metric tag** in the Datadog extension settings. Knock includes this tag when a [tenant](/concepts/tenants) is associated with the workflow run or message. Integration source metrics do not include a tenant tag.
