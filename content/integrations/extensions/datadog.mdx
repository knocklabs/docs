...
To add a `tenant:<tenant_id>` tag to workflow and message metrics, enable **Include tenant as a metric tag** in the <a href="https://dashboard.knock.app/~/settings/integrations/extensions?extension-datadog=true" target="_blank" rel="noopener">Datadog extension settings</a>. Knock includes this tag when a [tenant](/concepts/tenants) is associated with the workflow run or message. Integration source metrics do not include a tenant tag.
...
<Callout
  ...
  text="Tenant IDs can produce a large number of unique tag values. Enabling this setting increases metric cardinality and may increase your Datadog costs. Keep this setting off unless you need to filter or group Knock metrics by tenant."
/>
...
3. Enter a Datadog API Key from Datadog's <a href="https://app.datadoghq.com/organization-settings/api-keys" target="_blank" rel="noopener">API Keys</a> page (we recommend creating a dedicated key just for Knock)
4. Pick the correct site for your Datadog account (visit <a href="https://docs.datadoghq.com/getting_started/site/" target="_blank" rel="noopener">Datadog's docs</a> for more information)
...
1. Visit <a href="https://app.datadoghq.com/dashboard/lists" target="_blank" rel="noopener">Datadog's dashboard list</a> and click "New Dashboard"
...
3. If you created a dedicated Datadog API key for Knock, you can now delete the key from Datadog's <a href="https://app.datadoghq.com/organization-settings/api-keys" target="_blank" rel="noopener">API Keys</a> page
