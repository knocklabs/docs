...
<Callout
  ...
  text={
    <>
      Amazon SES tags are limited to 256 characters and can
      only contain ASCII letters (a-z, A-Z), numbers (0-9), underscores (_), or
      dashes (-). If your <code>knock_recipient_id</code> does not meet these
      requirements, Knock will truncate <code>knock_recipient_id</code> to 256
      characters and remove any prohibited characters. See the{" "}
      <a
