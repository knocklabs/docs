    ...
    **Knock settings**
    <Attributes>
      ...
      <Attribute
        name="CSS inlining"
        type="boolean"
        description="Whether Knock will inline CSS styles in your emails onto their associated HTML elements before sending, to improve email client compatibility. Defaults to true."
      />
    </Attributes>
    <br />
...
<Callout
  type="warning"
  title="Amazon SES tag limitations."
  text={
    <>
      ...
      <code>knock_recipient_id</code> does not meet these requirements, Knock
      will truncate <code>knock_recipient_id</code> to 256 characters and remove
...
<Steps titleSize="h3">
  ...
  <Step title="Configure SES to publish to SNS">
    ...
    4. Under **Feedback notifications**, click "Edit" and select your SNS topic for each feedback type:
...
When a recipient marks an email as spam, SES publishes a `Complaint` notification to your SNS topic. Knock records a [`message.complaint`](/developer-tools/outbound-webhooks/event-types#messagecomplaint) event on the message. A complaint does not change the message's delivery status, because the message was delivered before the recipient reported it.

You can see complaint events in the message timeline in the Knock dashboard, and subscribe to them with [outbound webhooks](/developer-tools/outbound-webhooks/overview).

Knock handles two special cases in SES `Complaint` notifications:

| SES complaint notification                                                    | Knock behavior                                                                                                                                                               |
| ----------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `complaintFeedbackType` is `not-spam`                                         | Ignored. The receiving mailbox provider retracted an earlier complaint, so Knock does not record an event.                                                                   |
| `complaintSubType` is `OnAccountSuppressionList` or `OnTenantSuppressionList` | Recorded as a bounce. SES did not attempt delivery because the address is on a suppression list, so Knock updates the message to `bounced` instead of recording a complaint. |
