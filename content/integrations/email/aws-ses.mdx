...
Knock handles two special cases in SES `Complaint` notifications, as follows:

- **`complaintFeedbackType` is `not-spam`**: Knock ignores the notification. The receiving mailbox provider retracted an earlier complaint, so Knock does not record an event.
- **`complaintSubType` is `OnAccountSuppressionList` or `OnTenantSuppressionList`**: Knock records a bounce. SES did not attempt delivery because the address is on a suppression list, so Knock updates the message to `bounced` instead of recording a complaint.
