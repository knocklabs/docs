...
<Steps titleSize="h3">
  ...
  <Step title="Configure webhooks in Postmark">
    ...
    2. Select the message stream you're sending with (usually the default transactional stream) and open the **Webhooks** tab
    3. Click "Add webhook" and paste the webhook URL from Knock
    4. Select the events to send to Knock:
       - **Delivery** - Tracks successful delivery
       - **Bounce** - Tracks bounce events
       - **Spam complaint** - Tracks spam complaints reported by recipients
    5. Click "Save webhook"
    6. Postmark will automatically begin sending events to Knock
...
When a recipient marks an email as spam, Postmark sends a `SpamComplaint` webhook event to Knock. Knock records a [`message.complaint`](/developer-tools/outbound-webhooks/event-types#messagecomplaint) event on the message. A complaint does not change the message's delivery status, because the message was delivered before the recipient reported it.

You can see complaint events in the message timeline in the Knock dashboard, and subscribe to them with [outbound webhooks](/developer-tools/outbound-webhooks/overview). Postmark deactivates the recipient's address after a spam complaint, so later sends to that address bounce.

### Troubleshooting

If delivery status updates aren't appearing in Knock:

1. **Check webhook configuration.** Verify the Delivery, Bounce, and Spam complaint events are enabled on the webhook with the correct Knock webhook URL in your Postmark message stream settings.
