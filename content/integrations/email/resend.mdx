...
<Steps titleSize="h3">
  ...
  <Step title="Add webhook endpoint in Resend">
    ...
    4. Select the events you want to track:
       ...
       - **email.complained** - Tracks spam complaints reported by recipients
...
When a recipient marks an email as spam, Resend sends an `email.complained` webhook event to Knock. Knock records a [`message.complaint`](/developer-tools/outbound-webhooks/event-types#messagecomplaint) event on the message. A complaint does not change the message's delivery status, because the message was delivered before the recipient reported it.

You can see complaint events in the message timeline in the Knock dashboard, and subscribe to them with [outbound webhooks](/developer-tools/outbound-webhooks/overview). Resend does not include a complaint reason in the event.
...
2. **Verify event selection.** Ensure you've enabled the `email.delivered`, `email.bounced`, and `email.complained` events.
