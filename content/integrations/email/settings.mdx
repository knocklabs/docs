...
As an example, if you wanted to send all emails to a single address, you could set the `to` field to that address at the channel to either a static value (like `hello@example.com`) or a dynamic value (like `{{ data.email_to_override }}`).
