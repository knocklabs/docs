...
The legacy `.button` class uses `primary_color` for the background and border, and `primary_color_contrast` for the text. In dark mode, these are overridden with `dark_primary_color` and `dark_primary_color_contrast`:

```css title="Legacy button styling"
.button {
...
The visual editor renders button colors as static inline `style` attributes, which don't change between light and dark mode. Without layout-level overrides, buttons would appear with their light-mode colors in dark mode, making them unreadable against dark backgrounds.

To solve this, the default layout uses `!important` in dark mode to override the inline styles injected by the visual editor. This means:
