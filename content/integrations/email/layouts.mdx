...
Layouts can also use [MJML](/integrations/email/mjml). An MJML layout must have a root `<mjml>` tag instead of an HTML document, and any plain HTML inside must be wrapped in `<mj-raw>` tags.
...
@media (prefers-color-scheme: dark) {
  .block-row--button_set-v1 .block-button--solid {
    background-color: {{ vars.branding.dark_primary_color | default: "#FFFFFF" }} !important;
    color: {{ vars.branding.dark_primary_color_contrast | default: "#000000" }} !important;
  }

  .block-row--button_set-v1 .block-button--outline {
    background-color: transparent !important;
    color: {{ vars.branding.dark_primary_color | default: "#FFFFFF" }} !important;
    border-color: {{ vars.branding.dark_primary_color | default: "#FFFFFF" }} !important;
  }
}
```
