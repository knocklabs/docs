...
For push-specific sending needs, you can configure a set of overrides per template. To do this, click the gear icon at the top of the template editor to access the template settings modal. These overrides are merged into the push payload sent to the underlying provider and can be used to set badge counts, custom sound files, and other provider-specific settings.
