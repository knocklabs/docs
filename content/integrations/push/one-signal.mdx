...
The following channel settings should be configured per [environment](/concepts/environments). Navigate to **Channels and sources** in your dashboard account settings, select your OneSignal [channel](/concepts/channels), then click "Manage configuration" under the environment that you'd like to configure.
...
<AccordionGroup>
  <Accordion title="Settings" anchorSlug="configuration-settings">
    ...
    **Knock settings**
    <Attributes>
      <Attribute
        ...
        description="Whether to enable sandbox mode for your OneSignal channel."
      />
      <Attribute
    ...
    **Provider settings for OneSignal**
    <Attributes>
      <Attribute
        name="App ID"
        type="string*"
        description="The unique app ID of your OneSignal project."
      />
      <Attribute
        name="API Key"
        type="string*"
        description="The private API key from OneSignal."
      />
      <Attribute
        name="Recipient mode"
        type="enum*"
        description="The mode used to target recipients. One of External ID or Player ID."
      />
    </Attributes>
...
We have full support for overriding the payload sent to OneSignal for adding things like badge counts, extra data properties, and sound files.
...
<AccordionGroup>
  <Accordion
    ...
    anchorSlug="faq-player_ids-and-external_ids-together"
  >
    No, you can currently only use one or the other.
  </Accordion>
  <Accordion
    title="Can I use a different identifier as an `external_id`?"
    anchorSlug="faq-custom-external_id-identifier"
  >
    Currently this is not supported but we'd love to hear a use case you have
    for it!
  </Accordion>
  <Accordion
    title="Can I use a `template_id` stored in OneSignal?"
    anchorSlug="faq-onesignal-template_id"
  >
    Yes, you can use push overrides to override the JSON payload and specify the
    template_id for OneSignal instead.
  </Accordion>
  <Accordion
    title="Can I use the OneSignal Push integration to send email or SMS?"
    anchorSlug="faq-onesignal-push-for-email-or-sms"
  >
