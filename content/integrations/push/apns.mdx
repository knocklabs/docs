...
We support sending [APNS notifications as "silent", data-only notifications](https://developer.apple.com/documentation/usernotifications/setting_up_a_remote_notification_server/pushing_background_updates_to_your_app) within Knock. You can enable this per push notification template by tapping the 'Manage template settings' button in the header of the template editor, and then toggling on the "Send as a silent notification" option.
...
## Deep Links and Universal Links Integration

Our system also supports the integration of deep links and [universal links](https://developer.apple.com/ios/universal-links/) within your push notifications, catering to your specific routing strategies. To incorporate these links, simply include them in the payload overrides section as previously mentioned. This feature ensures seamless navigation and enhances user engagement by directing users to specific content or pages within your app directly from the notification.

![Manage template settings](/images/integrations/push/manage-template-settings.png)
...
1. All data included in the sender's message payload will be accessible to the client receiving the push notification.
2. Apple imposes a limit of 256 bytes for the total APNS payload. Notifications exceeding this size will not be delivered.

## Debugging

If you've followed all setup steps but still aren't receiving notifications on your device, consider the following:
1. Verify your APNS Channel configuration, particularly, which `Application Mode` you have selected.
    - Select `Production` for builds distributed through the App Store and TestFlight. Choose `Sandbox` for builds compiled via Xcode.
    ![Manage template settings](/images/integrations/push/apns-configuration.png)
2. Utilize [Apple's Push Notification Console](https://developer.apple.com/documentation/usernotifications/testing_notifications_using_the_push_notification_console) to dispatch test notifications to your app directly.
    - Successfully receiving a test notification from this console indicates a potential issue with your APNS configuration in Knock.
    - If a test notification fails to reach your device, it suggests a problem with the APNS setup within your app, or an error in the creation of your token/certificate.
