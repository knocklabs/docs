...
The Segment source enables you to receive [Segment webhook events](https://segment.com/docs/connections/webhooks/) directly in Knock. Segment sends webhook callbacks when track and identify events flow through your workspace. Knock verifies each payload using your Segment webhook signing secret, identifies the event type, and executes the actions you configure.
...
<Steps>
  ...
  <Step title="Select default action mappings">
    Once you've selected **Segment** as a source, you can select your desired action mappings. You can always add additional action mappings later. Click the **Connect Segment** button to continue.
...
Segment sends events based on the [Segment spec](https://segment.com/docs/connections/spec/). Below are common event types you might map to actions in Knock.
...
See the [Segment spec documentation](https://segment.com/docs/connections/spec/) for the full list of available event types and their schemas.
