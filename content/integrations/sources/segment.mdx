...
<Steps>
  ...
  <Step title="Configure webhook">
    ...
    <AccordionGroup>
      <Accordion title="Select data source">
        Check the data source you'd like to receive events from and click "Next."
      </Accordion>
  ...
  <Step title="Save changes">
    Now you'll see a **Basic Settings** page. Check the "Enable Destination" button and click "Save changes."
  </Step>
  <Step title="Add mapping">
    At the top of the page, navigate to the **Mappings** tab. Click "New Mapping", and under Actions, click "Send".

    You will then be directed to a form to configuring the webhook:
    <AccordionGroup>
      <Accordion title="Select events to map and send">
        If you'd like to send all events to Knock, you can add the condition **Event type is Track** and a condition for **Event type is Identify.** Make sure you change the top operator to **any** instead of all.
        ![Select event conditions.](/images/webhooks-select-events.png)
      </Accordion>
      <Accordion title="Add test event">
      ...
      <Accordion title="Select mappings">
        Paste the URL you copied from the Knock dashboard for the desired source environment. A dropdown list of event variables and functions will appear, but you do not need to select anything.
      </Accordion>
      <Accordion title="Send test event">
...
Actions are automatically enabled when you create them. To stop an event from triggering an action, go to the action page and toggle its status to **Inactive.** Keep in mind that this will disable that action for the current environment only. When you're ready to trigger the action again, you can set it back to **Active.**
...
<AccordionGroup>
  <Accordion title="Can I have one event trigger multiple workflows?">
  ...
  <Accordion title="Can I have multiple events trigger one workflow?">
    Yes. To do so, you will follow the steps above to create an action for the
    additional event and select the same workflow from the **Create workflow
