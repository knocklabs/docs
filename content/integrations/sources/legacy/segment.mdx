...
<Callout
  ...
  text={
    <>
      ...
      <a href="/integrations/sources/segment">Segment source</a> instead.
    </>
  }
/>
...
You can add a **track event** as a trigger to your workflow directly from the workflow builder. Click on the workflow's trigger step and set the trigger type to "Source Event." Then you'll be able to select the event and map its properties into the data the workflow needs. The workflow can still be triggered via the API at any time.
