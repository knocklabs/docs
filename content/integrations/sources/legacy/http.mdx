...
Received events can be configured as workflow triggers directly in the workflow editor. Click on the workflow's trigger step and set the trigger type to "Source Event." Then you can select the event and map its properties into the data the workflow needs. The workflow can still be triggered via the API at any time.
