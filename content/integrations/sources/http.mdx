...
The HTTP source creates a generic event ingestion endpoint that you can use to stream events from your service, or a third-party platform into Knock. The events you stream into the HTTP source can then be used to trigger Knock notification workflows via configured workflow triggers.
...
Received events can be mapped to corresponding workflow triggers under each environment's configured source.
...
You can see a log of all of the events received per source under **Developers** > **Sources** in the Knock dashboard under the "Logs" page for your configured source. You can also see any workflow triggers that were activated as part of the event ingestion, and any workflow runs that were triggered.
