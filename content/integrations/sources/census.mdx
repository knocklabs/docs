...
Follow the steps below to add a user data sync to your configured Knock HTTP request destination in Census. Note that while these instructions leverage our bulk identify endpoint to optimize migrating large numbers of user records to Knock, you can adapt this approach to use our [identify endpoint](/api-reference/users/identify) for smaller batches of users and ongoing syncs.
