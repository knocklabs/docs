...
description: Learn how to power your Knock notification system with incoming webhooks, customer data platforms, reverse ETLs, and your data warehouse.
section: Integrations > Sources
layout: integrations
---

With Knock sources, you can connect external services to Knock to trigger notification workflows, identify users, and automate other actions. Knock supports three types of sources:

- **Incoming webhooks.** Receive webhooks from third-party services like Stripe, Clerk, and PostHog, then map events to actions in Knock. Knock offers [pre-built integrations](/integrations/sources/incoming-webhooks/overview) with verification and event mappings ready to use, as well as [custom webhooks](/integrations/sources/incoming-webhooks/custom-webhooks) for connecting any service.
- **Customer data platforms (CDPs).** Integrate platforms like [Segment](/integrations/sources/segment) and [RudderStack](/integrations/sources/rudderstack) to pipe track and identify events into Knock.
- **Reverse ETLs.** Sync customer data from your data warehouse using [Hightouch](/integrations/sources/hightouch), [Census](/integrations/sources/census), or [Polytomic](/integrations/sources/polytomic).
...
- **Minimize engineering customization.** If you already use a CDP, reverse ETL, or a service that supports webhooks, you can start sending events to Knock in minutes without writing custom integration code.
- **Ensure consistency.** Sources keep customer data synchronized between services, including Knock. Email, name, and other user trait updates can flow into Knock without additional engineering work.

## Concepts

### Sources

A source is any platform that can pass event and/or user data into Knock. Sources can be incoming webhook integrations, CDPs (such as [Segment](https://segment.com) and [RudderStack](https://rudderstack.com)), or reverse ETLs (such as [Hightouch](https://hightouch.com/) and [Census](https://www.getcensus.com/)). The incoming events from these services can be used to orchestrate actions in Knock such as creating/updating users, triggering workflows, managing objects, and more.

Knock currently supports the following sources:

**Incoming webhooks**

- [Clerk](/integrations/sources/incoming-webhooks/clerk)
- [PostHog](/integrations/sources/incoming-webhooks/posthog)
- [Stripe](/integrations/sources/incoming-webhooks/stripe)
- [Supabase](/integrations/sources/incoming-webhooks/supabase)
- [WorkOS](/integrations/sources/incoming-webhooks/workos)
- [Custom webhooks](/integrations/sources/incoming-webhooks/custom-webhooks) (connect any service that supports webhook delivery)
...
- [HTTP](/integrations/sources/http) (legacy generic HTTP endpoint for accepting events)
...
Incoming webhooks enable you to connect third-party services to Knock by receiving their webhook events and mapping them to actions. Unlike CDP sources that use a standardized track/identify event spec, incoming webhooks work with any webhook payload format and support a broader set of actions including triggering workflows, managing users, updating objects, and more.

Knock provides pre-built integrations for popular services like [Stripe](/integrations/sources/incoming-webhooks/stripe), [Clerk](/integrations/sources/incoming-webhooks/clerk), [WorkOS](/integrations/sources/incoming-webhooks/workos), [Supabase](/integrations/sources/incoming-webhooks/supabase), and [PostHog](/integrations/sources/incoming-webhooks/posthog). These come with webhook verification and event-to-action mappings pre-configured, so you can get started by pasting a signing secret and toggling on the mappings you need.

For services without a pre-built integration, you can use [custom webhooks](/integrations/sources/incoming-webhooks/custom-webhooks) to connect any service that supports webhook delivery.

[Learn more about incoming webhooks →](/integrations/sources/incoming-webhooks/overview)

## CDP and reverse ETL sources

The following sections apply to customer data platform (CDP) and reverse ETL sources such as Segment, RudderStack, Hightouch, and Census.
