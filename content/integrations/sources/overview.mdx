...
![A screenshot of where to configure idempotency for your Source.](/images/sources/idempotency.png)
...
Your idempotency keys must be valid strings no more than 255 characters in length. If an invalid key is found, Knock will still ingest your source event, but it will not attempt to execute your event idempotently. In addition, the invalid idempotency key will be dropped—you will not see it appear in your event logs.
