...
Knock offers pre-built integrations for popular services such as [Clerk](/integrations/sources/clerk), [Stripe](/integrations/sources/stripe), [PostHog](/integrations/sources/posthog), [Supabase](/integrations/sources/supabase), and [WorkOS](/integrations/sources/workos). These integrations provide automatic signature verification and pre-configured event types. For any service that can send HTTP callbacks, you can use the [HTTP source](/integrations/sources/http) to create a custom webhook integration with full control over event type identification and field mappings.

## CDP sources

CDP sources send `track` and `identify` events from customer data platforms into Knock. Use a CDP source when you already have event instrumentation in a platform like [Segment](/integrations/sources/segment) or [RudderStack](/integrations/sources/rudderstack) and want to use those events to trigger workflows and keep user data in sync without additional API calls from your application.
...
Reverse ETL sources sync warehouse data into Knock using direct API calls or embedded destinations. Platforms like [Hightouch](/integrations/sources/hightouch), [Census](/integrations/sources/census), and [Polytomic](/integrations/sources/polytomic) can push user, object, and tenant data from your data warehouse into Knock on a scheduled basis.
