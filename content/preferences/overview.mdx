...
<AccordionGroup>
  ...
  <Accordion title="What happens if a user changes their preferences during an extended workflow run?">
    Knock evaluates a user's preference set each time a workflow run reaches an eligible channel step, so some preference changes (like a channel opt-out) can take effect mid-run. However, each workflow run is pinned to the workflow version active when it started. As a result, checks based on workflow metadata (such as category or commercial status) will always reference the metadata from the pinned workflow version. Changes to workflow metadata will only impact new workflow runs.
