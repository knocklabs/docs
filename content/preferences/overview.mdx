...
When Knock reaches a valid channel step in a workflow run for a user, we evaluate their `PreferenceSet`. A message will not send if the user has opted out of receiving it.
...
<AccordionGroup>
  ...
  <Accordion title="What is the merge strategy, and how can I use it to override the merge hierarchy?">
    ...
    - The `__strategy__` key can only be used on a combined `workflows`-`channel_types`, `workflows`-`channels`, `categories`-`channel_types`, or `categories`-`channels` preference, as seen in the example above.
  ...
  <Accordion title="What happens if a user changes their preferences during an extended workflow run?">
    Knock evaluates a user's preference set each time a workflow run reaches a valid channel step, so certain changes to a preference set will be respected. However, each workflow run is pinned to a particular version of a workflow when the workflow run starts. If you make changes to a workflow's metadata properties, like `categories` or `is_commercial`, user preferences that reference that metadata will be evaluated using the values used in the workflow version that was live when the workflow run was started. For example, updating a workflow to `is_commercial` will not retroactively impact existing workflow runs.
