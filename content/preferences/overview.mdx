...
## Choosing a preference center

A preference center is a place where users manage their notification preferences. Knock offers two approaches: a hosted, no-code page you configure in the dashboard, or a custom UI you build and embed in your application.

<Table
  headers={["", "Hosted preference center", "Build your preference center"]}
  rows={[
    ["Who hosts the UI", "Knock", "Your application"],
    [
      "Configuration",
      "Dashboard (rows, labels, branding)",
      "Your code (optionally dashboard-managed config)",
    ],
    [
      "Access pattern",
      "Signed URL per user",
      "Public API key + signed user token",
    ],
    [
      "Supported scopes",
      "User default preferences only",
      "User, tenant, and object preferences; non-default preference sets",
    ],
    ["Custom subdomain", "Yes", "N/A (embedded in your product)"],
    ["Code required", "No", "Yes"],
  ]}
/>

Use these questions to choose an approach:

1. **Do you want Knock to host the UI with no code?** Do you want dashboard-managed rows, labels, and branding, with optional custom subdomain support? See the [hosted preference center](/preferences/hosted-preference-center).
2. **Do you need to generate a hosted preference center link outside of workflow templates?** For example, from your app settings page or an admin panel. See [Get a signed URL from your backend](/preferences/hosted-preference-center#get-a-signed-url-from-your-backend).
3. **Do you want to embed the preference center in your product for full visual control?** You can define row structure in code, or optionally reuse the dashboard-managed config shape. See [Build your preference center](/preferences/build-your-preference-center).
...
<AccordionGroup>
  <Accordion title="Standard preference merge hierarchy">
    ...
    2. The [environment-level default preference](/preferences/build-your-preference-center#create-a-default-preferenceset) set
  ...
  <Accordion title="Tenant-specific preference merge hierarchy">
    ...
    3. The [environment-level default preference](/preferences/build-your-preference-center#create-a-default-preferenceset) set
...
<>
  If a <code>tenant</code> is applied to the workflow trigger but there are no
  tenant-specific preferences set on the tenant or the recipient, the
  recipient's <code>default</code> preference set will be evaluated according to
  the standard merge hierarchy, as if no <code>tenant</code> were applied.
</>
} />
...
<AccordionGroup>
  ...
  <Accordion title="What's the difference between channel preferences and channel type preferences?">
