...
<AccordionGroup>
  ...
  <Accordion title="Tenant-specific preference merge hierarchy">
    ...
    />
    <br />
    To visualize the resulting merged preferences, we'll start at the bottom of the hierarchy and work our way up, adding any preferences from the environment default to the tenant's default preference set, then adding any of these preferences that are not already present in the recipient's tenant-specific preference set. At each merge point, any preferences that are set in both places will keep the value from the preference set that is higher up in the hierarchy:
