...
<Steps titleSize="h3">
  ...
  <Step title="Render your preference center">
    ...
    Once you have loaded a user's preference, you'll need to render an interface in your application so they can update their notification preferences. You can see a non-functional example of this below, but our [quickstart guide on building preferences UI](/in-app-ui/react/preferences) provides a more in-depth example.

```jsx title="A basic preference center"
<form
onSubmit={(e)=>{
  e.preventDefault();
  await updatedUserPreferences();
}}
>
  <label>
    Email
    <input type="checkbox" value={preferences.channel_types.email} />
  </label>
  <label>
    In-app
    <input type="checkbox" value={preferences.channel_types.in_app_feed} />
  </label>
