...
You can combine these keys to create preference grids (<a href="https://knock-in-app-notifications-react.vercel.app/preferences" target="_blank">example</a>):
...
There are two steps to building a preference center with Knock:
...
<Steps titleSize="h3">
  ...
  <Step title="Render your preference center">
    ...
    You can use the `getPreferences` method to retrieve a user's preferences and render them in your application. If no preferences are set, this method will return the default preference set you created in the step above.
    ...
    <Callout
      ...
      text={
        <>
          ...
          <a href="https://github.com/knocklabs/knock-node-example-app/blob/main/app/users/components/NotificationPreferencesModal.tsx#L42" target="_blank">
            getting
          </a>{" "}
          and{" "}
          <a href="https://github.com/knocklabs/knock-node-example-app/blob/main/app/users/components/NotificationPreferencesModal.tsx#L93" target="_blank">
            setting
          </a>{" "}
...
<Callout
  ...
  text={
    <>
      ...
      <a href="https://github.com/knocklabs/knock-node-example-app/blob/main/db/seeds.ts#L103" target="_blank">
        our example app
      </a>{" "}
...
<AccordionGroup>
  <Accordion title="General preference rules">
    ...
    - A recipient's preferences are merged against the default `PreferenceSet` within a given environment, with recipient-level preferences taking precedence in the merge.
  </Accordion>
...
- [Per-tenant preferences.](/preferences/per-tenant-preferences) In multi-tenant B2B applications, an advanced use case is customer admins who want to set the tenant-level default `PreferenceSet` for new users within their tenant.
