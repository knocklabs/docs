...
Knock evaluates a recipient's `PreferenceSet` for each channel step during a workflow run. A message will not send if the user has opted out of receiving it.
...
<AccordionGroup>
  ...
  <Accordion title="What happens if a user changes their preferences during a paused workflow run?">
    Knock evaluates a user's preference set each time a workflow run reaches an eligible channel step, so some preference changes (like a channel opt-out) can take effect mid-run. However, each workflow run is pinned to the workflow version that was current when it was triggered. As a result, checks based on workflow properties (such as category or commercial status) will always reference the properties from the pinned workflow version. Changes to workflow properties will only impact new workflow runs.
