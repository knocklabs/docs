...
The following hierarchies are used when merging preferences. Each item in the list takes precedence over the ones that follow it.

<AccordionGroup>
  <Accordion title="When no tenant is applied to the workflow trigger">
    - A recipient's `default` preference set
    - The environment-level default preference set

    Let's take a look at an example to visualize this merge hierarchy. Suppose that you have the following preference sets:
    {/* add example images */}
  ...
  <Accordion title="When a tenant is applied to the workflow trigger">
    ...
    <Callout
      ...
      text={
        <>
          If a <code>tenant</code> is applied to the workflow trigger but there are no tenant-specific preferences set on the tenant or the recipient, the recipient's <code>default</code> preference set will be evaluated as if no <code>tenant</code> were applied.
        </>
      }
    />
