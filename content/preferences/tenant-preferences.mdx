...
Use tenant preferences to enable your customers' admins to create a tenant-specific default `PreferencesSet` for users in their tenant.

If you're a B2B application or a multi-tenant SaaS product, you can use tenant preferences to allow your customers to set default preferences for their users. A well-known example of this: in Slack, your notification preferences are set _per Slack workspace_ (that is, per-tenant), not as global preferences that apply across all of your Slack workspaces.
...
## Create per-tenant default 'PreferenceSet'
...
## Set per-tenant user `PreferenceSet`

A `PreferenceSet` has an `id`. When you [set a given user's preferences](/reference#set-preferences-user) in Knock, you set this to `default` when you want to apply the preferences universally for the user. (If you don't provide an `id`, the `default` preference set is used.)
...
You can also set `id` to a `tenant.id` to create a `tenant`-specific preference set for the user.
...
In summary, the tenant-specific preference evaluation rules are as follows:
...
- When executing a workflow trigger, passing in a `tenant` will automatically load that tenant's default `PreferencesSet` (if one exists) for all recipients of the workflow.
- If the recipient has any per-tenant overrides set for that tenant, they override the default preferences set at the tenant level.
- If there is no default `PreferenceSet` on the tenant AND the recipient has no per-tenant preferences set, the recipient’s `default` id `PreferenceSet` will be used.
