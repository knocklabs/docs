...
As an alternative to the push, commit, and promote workflow, you can use the `--force` flag on push commands to bypass environment restrictions and push directly to non-development environments (for example, staging or production). When using `--force` in this way, you should also pass `--commit` so that your changes are persisted. See the [push command reference](/cli/resources/push) for details.
