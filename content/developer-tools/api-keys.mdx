...
In Knock, all requests to the [Knock API](/api-reference) are issued using a API key. Your API keys are [environment](/concepts/environments) specific and allow Knock to tie a request on the API back to an isolated Knock environment.
...
<AccordionGroup>
  <Accordion title="Can I revoke an API key once generated?">
    ...
    navigate to **Platform** > **API keys** in your dashboard sidebar, select the "..."
    menu next to the key you want to rotate, and click "Roll API key." This will
    prompt you to confirm the operation, which will immediately issue a new key
    and invalidate the previous key. This action cannot be undone.
  </Accordion>
