...
description: Learn how the Agent Toolkit exposes your workflows as tools that your AI Agents can invoke to power cross-channel user messaging, without any integration logic.
...
When using the agent toolkit, you can optionally configure exactly which workflows are exposed as tools. To do so, you can set the `permissions.workflows.trigger` property on the agent toolkit configuration. By default, no workflows are exposed as tools.
