...
The Agent Toolkit exposes a set of helper methods that make it trivial for you to wrap your tool calls in requiring human input to proceed, and leveraging Knock's webhook mechanism to handle the asynchronous nature of a human-in-the-loop interaction. Knock takes care of all of the heavy lifting for you, making it easy to build rich human-in-the-loop flows with little effort.
...
The Knock Agent Toolkit exposes a set of helper methods that you can use to power human-in-the-loop flows for common cases, and low-level primitives should you need to implement a custom flow.
