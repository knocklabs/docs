...
The Knock Agent Toolkit allows you to power rich human-in-the-loop interactions. For example, you can use the agent toolkit to create a workflow that lets a person to approve or reject work that the agent has done, powered by Knock's cross-channel messaging workflows.

While it's possible to build these flows on top of Knock as is, the agent toolkit exposes helpers to help you build these human-in-the-loop workflows more easily in your agent applications.
...
2. Your agent calls to Knock to [trigger the workflow](/send-notifications/trigger-workflows/overview) with a set of recipients and any additional context for the request.
3. Your user receives the message on the channel, which includes a the context for the request and a set of actions to take.
4. The user then interacts with the message and the result is recorded back to Knock.
...
The Knock Agent Toolkit exposes a set of helper methods that you can use to power human in the loop flows.
...
const classifyToolRequiringInput = toolkit.requireHumanInput(classifyWork, {
  ...
  recipients: ["user_123"],
  // (optional) Any additional data to pass
  data: {}
});
```
...
function handleIncomingInteraction(event) {
  ...
  if (interactionEvent && interactionEvent.workflow === "my-approval-workflow" && interactionEvent.action === "approved") {
...
You pass the `resumeToolExecution` method the `message.interacted` event that you've received from Knock via the `handleMessageInteracted` method. Here's an example of how you might use this method:
...
<Steps>
  ...
  <Step title="Wrapping a tool as requiring human input">
    ...
    const wrappedClassificationTool = toolkit.requiresHumanInput(classificationTool, {
  ...
  <Step title="Handling the response and resuming tool execution">
    ...
    const handleMessageInteracted = async (event: MessageInteractedEvent) => {
      // Handle the response from the human in the loop
    }
    ```
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="Can I power a direct approve/reject via email without routing the buttons through my server?">
    No, right now you must setup a URL handler on your server to implement these actions. One possible approach is that you drive the resulting approve or reject click directly to your agent workflow to handle.
  </Accordion>
  <Accordion title="Can I power a direct approve/reject workflow via Slack or MS Teams without routing the interaction through my server?">
    No, as with email you must implement a webhook handler for these channels where you handle this interaction before sending it back to Knock to proceed.
  </Accordion>
</AccordionGroup>