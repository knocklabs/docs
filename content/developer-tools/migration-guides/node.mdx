...
5. The `signUserToken` method is now async and has updated usage (see [full details below](/developer-tools/migration-guides/node#signing-user-tokens))
   - Must use `await`: returns `Promise<string>` instead of `string`
   - Options object: Uses single options object instead of positional arguments
   - Parameter rename: `expiresInSec` is now `expiresInSeconds`
   - Default TTL: Tokens now explicitly expire after 3600 seconds by default
...
// Synchronous operation with positional arguments
const token = signUserToken("user-1");

// With optional parameters (signing key and expiry)
const tokenWithExpiry = signUserToken(
  "user-1",
  knockSigningKey,
...
// Now async and requires options object
const token = await Knock.signUserToken("user-1", {
  signingKey: knockSigningKey, // Required if not set via KNOCK_SIGNING_KEY env variable
});

// With optional parameters using new naming
const tokenWithExpiry = await Knock.signUserToken("user-1", {
  signingKey: knockSigningKey,
  expiresInSeconds: 3600, // Note: expiresInSeconds instead of expiresInSec
});

// With token grants
import { buildUserTokenGrant, Grants } from "@knocklabs/node/lib/tokenSigner";

const tokenWithGrants = await Knock.signUserToken("user-1", {
  grants: [
    buildUserTokenGrant({ type: "tenant", id: "org_123" }, [
