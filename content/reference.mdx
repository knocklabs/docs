...
<Callout
  ...
  text={
    <>
      Note: this API endpoint uses the <code>conversation.list</code> endpoint
      from the Slack API under the hood. This Slack endpoint has its own{" "}
      <a href="https://api.slack.com/apis/rate-limits">rate limits</a> of 20
      requests per minute per account per token.
    </>
  }
/>
### Endpoint
