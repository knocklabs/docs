...
<Attributes>
  ...
  <Attribute
    name="messge_ids[]"
    type="string[] (optional)"
    description="Limits the results to only the message ids given (max 50). Note: when using this option, messages returned will also match any other filters applied."
  />
  <Attribute
