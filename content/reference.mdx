...
<Section title="Microsoft Teams" slug="ms-teams">
  <ContentColumn>
  These endpoints handle communication with the Microsoft Graph API to streamline authentication and data gathering. They are used by [TeamsKit](/integrations/chat/teams-kit/overview) components and hooks.
  </ContentColumn>
...
- `200` Success response
- `403` When no Microsoft Teams tenant ID is set for the given object.
...
```json
// Missing Microsoft Teams tenant ID error response
...
<ContentColumn>
Removes a Microsoft Teams tenant ID from the Microsoft Teams tenant object.
