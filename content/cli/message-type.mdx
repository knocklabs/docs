...
<Attributes>
  ...
  <Attribute
    name="--template"
    type="string"
    description="A template to use for scaffolding the message type. Accepts a template string that resolves to a GitHub repository URL. For example, `<your-org>/<your-repo>/message-types/my-message-type`. Use `message-types/<message-type-key>` to reference [pre-built Knock templates](https://github.com/knocklabs/templates/tree/main/message-types)."
  />
  <Attribute
    name="--force"
    type="boolean"
...
```bash title="Create a message type from a template"
knock message-type new --key=my-message-type --template=message-types/my-message-type
```
...
<Section title="knock message-type open <message_type_key> [flags]" path="/message-type/open">
<ContentColumn>

Open a message type in the Knock dashboard. This command opens the message type page in your default browser.
...
```bash title="Basic usage"
knock message-type open my-message-type
```

```bash title="Open a message type in a different environment"
knock message-type open my-message-type --environment=production
```
