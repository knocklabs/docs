...
<Attributes>
  ...
  <Attribute
    name="--template"
    type="string"
    description="A template to use for scaffolding the partial. Accepts a template string that resolves to a GitHub repository URL. For example, `<your-org>/<your-repo>/partials/my-partial`. Use `partials/<partial-key>` to reference [pre-built Knock templates](https://github.com/knocklabs/templates/tree/main/partials). You cannot use this flag with --type."
  />
  <Attribute
    name="--force"
    type="boolean"
...
```bash title="Create a partial from a template"
knock partial new --key=my-partial --template=partials/my-partial
```
...
<Section title="knock partial open <partial_key> [flags]" path="/partial/open">
<ContentColumn>

Open a partial in the Knock dashboard. This command opens the partial page in your default browser.
...
```bash title="Basic usage"
knock partial open my-partial
```

```bash title="Open a partial in a different environment"
knock partial open my-partial --environment=production
```
