...
<Attributes>
  ...
  <Attribute
    name="--template"
    type="string"
    description="A template to use for scaffolding the guide. Accepts a template string that resolves to a GitHub repository URL. For example, `<your-org>/<your-repo>/guides/my-guide`. Use `guides/<guide-key>` to reference [pre-built Knock templates](https://github.com/knocklabs/templates/tree/main/guides). You cannot use this flag with --message-type."
  />
  <Attribute
    name="--force"
    type="boolean"
...
```bash title="Create a guide from a template"
knock guide new --key=my-guide --template=guides/my-guide
```
...
<Section title="knock guide open <guide_key> [flags]" path="/guide/open">
<ContentColumn>

Open a guide in the Knock dashboard. This command opens the guide page in your default browser.
...
```bash title="Basic usage"
knock guide open my-guide
```

```bash title="Open a guide in a different environment"
knock guide open my-guide --environment=production
```

</ExampleColumn>
</Section>

<Section title="knock guide generate-types [flags]" path="/guide/generate-types">
<ContentColumn>

Generate types for all guides in an environment and write them to a file. The target language is inferred from the output file extension. Supports `.ts`, `.py`, `.go`, and `.rb`.
...
<Attributes>
  <Attribute
    name="--environment"
    type="string"
    description="The environment to generate types for. Defaults to development."
  />
  <Attribute
    name="--branch"
    type="string"
  ...
  <Attribute
    name="--output-file"
    type="string"
    description="Required. The output file to write the generated types to. The file extension determines the target language."
  />
</Attributes>
...
knock guide generate-types \
  --output-file=./types/knock-guides.ts
```

```bash title="Generate Python types"
knock guide generate-types \
  --output-file=./types/knock_guides.py
```
