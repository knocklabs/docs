...
```
    ...
    Once you have created the `knock.json` file, all subsequent `knock pull` and `knock push` commands will use the `.knock` directory as the default target directory. The CLI finds `knock.json` by walking up from your current directory, but it resolves a relative `knockDir` against the current working directory rather than the location of `knock.json`; always run CLI commands from the directory that contains `knock.json`.

    If you need to specify a different target directory for a single command, you can use the `--knock-dir` flag on `knock pull` and `knock push`, or a resource-specific directory flag such as `--workflows-dir` on that resource's `pull`, `push`, and `validate` commands.
...
When you use `knock pull`, resources will be grouped by resource type within subdirectories, producing the following directory structure. The `schemas/` directory is managed separately by [`knock schema pull`](/cli/schema/pull) and [`knock schema push`](/cli/schema/push), and resolves to the same location when a `knock.json` file is present.
