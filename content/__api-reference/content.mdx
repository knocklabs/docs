...
<Table
  ...
  rows={[
    [
      "code",
      "string",
      "A stable, machine-readable code that identifies the error.",
    ],
...
Use `code` to handle an error in your application. The `message` may change to improve its explanation. See [error codes](/api-reference/overview/error-codes) for common codes and their resolution steps.

`type` groups codes into a category you can branch on before you handle a specific `code`. `authentication_error` covers credential and access problems, and `api_error` covers server-side failures.

```json title="401 authentication error"
{
...
To resolve this error, send a valid API key in the `Authorization` header and retry the request.
...
Retry a server error with exponential backoff. If the error persists, contact Knock support and include the response's `x-request-id` header.
