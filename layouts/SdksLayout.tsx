...
import { sdksSidebarContent } from "@/data/sidebar";

const SdksLayout = ({ frontMatter, sourcePath, children }) => {
  const router = useRouter();
  let paths = slugToPaths(router.query.slug);
  const showBreadcrumbs = router.asPath !== "/sdks/overview";
  ...
  const { breadcrumbs, nextPage, prevPage } = useMemo(() => {
    ...
    return getSidebarInfo(sidebarPaths, sdksSidebarContent);
  }, [paths]);
  ...
  return (
    <Page.Container>
      ...
      <Page.Wrapper>
        <Page.Sidebar content={sdksSidebarContent}></Page.Sidebar>
        <Page.Content>
          {breadcrumbs && showBreadcrumbs && (
            <Page.Breadcrumbs pages={breadcrumbs} />
          )}
          <Page.ContentHeader
